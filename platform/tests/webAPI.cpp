...
{
    auto inspectResult =
        makeRequest<std::array<double, 2>, core::Renderer::PickResult>(
            "inspect", {{0.5, 0.5}});
...
{
    CHECK_THROWS_AS((makeRequest<core::SchemaParam, std::string>("schema",
                                                                   {"foo"})),
                    std::runtime_error);
}
...
{
    std::string result = makeRequestJSONReturn<core::SchemaParam>(
        "schema", core::SchemaParam{"camera"});
