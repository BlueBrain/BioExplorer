...
struct JsonReaderStackItem
{
    enum State
    {
        BeforeStart, //!< An object/array is in the stack but it is not yet
                     //!< called by StartObject()/StartArray().
    ...
    if (!mError)
    {
        if (CURRENT.IsObject() && TOP.state == JsonReaderStackItem::Started)
        {
            ...
            if (memberItr != CURRENT.MemberEnd())
                STACK->push(
                    JsonReaderStackItem(&memberItr->value,
