...
TEST_F(RapidJson, SIMD_SUFFIX(ReaderParseInsitu_DummyHandler_ValidateEncoding))
{
    for (size_t i = 0; i < kTrialCount; i++)
    {
        ...
        Reader reader;
        EXPECT_TRUE(
            reader.Parse<kParseInsituFlag | kParseValidateEncodingFlag>(s, h));
    }
}
...
TEST_F(RapidJson, SIMD_SUFFIX(ReaderParseIterativePullInsitu_DummyHandler))
{
    for (size_t i = 0; i < kTrialCount; i++)
    {
        ...
        while (!reader.IterativeParseComplete())
        {
            if (!reader.IterativeParseNext<kParseDefaultFlags |
                                           kParseInsituFlag>(s, h))
                break;
        }
        EXPECT_FALSE(reader.HasParseError());
    }
...
size_t Traverse(const T& value)
{
    ...
    {
    case kObjectType:
        for (typename T::ConstMemberIterator itr = value.MemberBegin();
    ...
    case kArrayType:
        for (typename T::ConstValueIterator itr = value.Begin();
             itr != value.End(); ++itr)
            count += Traverse(*itr);
        break;
