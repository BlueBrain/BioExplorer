...
TEST(Document, Parse_Encoding)
{
    ...
    doc.Parse<kParseDefaultFlags, UTF8<>>(
        s2.c_str()); // VS2010 or below cannot handle templated function
                     // overloading. Use const char* instead.
#else
...
{
    typedef GenericDocument<UTF8<>, MemoryPoolAllocator<>,
                            MemoryPoolAllocator<>>
        DocumentType;
...
TEST(Document, UTF16_Document)
{
    ...
    json.Parse<kParseValidateEncodingFlag>(
        L"[{\"created_at\":\"Wed Oct 30 17:13:20 +0000 2012\"}]");
...
typedef ::testing::Types<CrtAllocator, MemoryPoolAllocator<>>
    MoveAllocatorTypes;
