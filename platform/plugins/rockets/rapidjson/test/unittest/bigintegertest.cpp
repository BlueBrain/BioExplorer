...
TEST(BigInteger, AddUint64)
{
    ...
    EXPECT_TRUE(BigInteger(RAPIDJSON_UINT64_C2(0xFFFFFFFF, 0xFFFFFFFF)) ==
                kUint64Max);
...
TEST(BigInteger, MultiplyUint64)
{
    ...
    c *= RAPIDJSON_UINT64_C2(0xFFFFFFFF, 0xFFFFFFFF);
    EXPECT_TRUE(BIGINTEGER_LITERAL(
                    "19085757395861596536664473018420572782123800") == c);
}
...
TEST(BigInteger, LeftShift)
{
    ...
    a <<= 256; // interShift == 0
    EXPECT_TRUE(BIGINTEGER_LITERAL(
                    "2135987035920910082395021706169552114602704522356652769947"
                    "041607822219725780640550022962086936576") == a);
}
