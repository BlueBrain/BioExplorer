...
#define TEST_HASHER(json1, json2, expected)                                   \
    {                                                                         \
        ...
        /*printf("%s: 0x%016llx\n%s: 0x%016llx\n\n", json1, h1.GetHashCode(), \
         * json2, h2.GetHashCode());*/                                        \
...
TEST(SchemaValidator, Hasher)
{
    ...
    TEST_HASHER("2147483648", "2147483648",
                true); // 2^31 can only be fit in unsigned
    TEST_HASHER("-2147483649", "-2147483649",
                true); // -2^31 - 1 can only be fit in int64_t
    TEST_HASHER("2147483648", "2147483648",
                true); // 2^31 can only be fit in unsigned
    TEST_HASHER("4294967296", "4294967296",
                true); // 2^32 can only be fit in int64_t
    TEST_HASHER("9223372036854775808", "9223372036854775808",
                true); // 2^63 can only be fit in uint64_t
    ...
    TEST_HASHER("{\"a\":1, \"b\":2}", "{\"b\":2, \"a\":1}",
                true); // Member order insensitive
    TEST_HASHER("{}", "null", false);
    TEST_HASHER("{}", "false", false);
    TEST_HASHER("{}", "true", false);
    TEST_HASHER("{}", "0", false);
...
#define INVALIDATE(schema, json, invalidSchemaPointer, invalidSchemaKeyword,  \
                   invalidDocumentPointer, error)                             \
    {                                                                         \
        INVALIDATE_(schema, json, invalidSchemaPointer, invalidSchemaKeyword, \
                    invalidDocumentPointer, error, SchemaValidator, Pointer)  \
    }

#define INVALIDATE_(schema, json, invalidSchemaPointer, invalidSchemaKeyword,  \
                    invalidDocumentPointer, error, SchemaValidatorType,        \
                    PointerType)                                               \
    {                                                                          \
        ...
        if (validator.GetInvalidSchemaPointer() !=                             \
            PointerType(invalidSchemaPointer))                                 \
        {                                                                      \
            StringBuffer sb;                                                   \
        ...
        if (strcmp(validator.GetInvalidSchemaKeyword(),                        \
                   invalidSchemaKeyword) != 0)                                 \
        {                                                                      \
            printf("GetInvalidSchemaKeyword() Expected: %s Actual %s\n",       \
                   invalidSchemaKeyword, validator.GetInvalidSchemaKeyword()); \
            ADD_FAILURE();                                                     \
        }                                                                      \
        if (validator.GetInvalidDocumentPointer() !=                           \
            PointerType(invalidDocumentPointer))                               \
        {                                                                      \
            StringBuffer sb;                                                   \
...
TEST(SchemaValidator, MultiType)
{
    ...
    INVALIDATE(
        s, "[\"Life\", \"the universe\", \"and everything\"]", "", "type", "",
        "{ \"type\": {"
...
TEST(SchemaValidator, OneOf)
{
    Document sd;
    sd.Parse(
        "{\"oneOf\": [{ \"type\": \"number\", \"multipleOf\": 5 }, { \"type\": "
        "\"number\", \"multipleOf\": 3 } ] }");
    SchemaDocument s(sd);
...
TEST(SchemaValidator, Ref)
{
    ...
    VALIDATE(s,
             "{\"shipping_address\": {\"street_address\": \"1600 Pennsylvania "
             "Avenue NW\", \"city\": \"Washington\", \"state\": \"DC\"}, "
...
TEST(SchemaValidator, String_Pattern)
{
    Document sd;
    sd.Parse(
        "{\"type\":\"string\",\"pattern\":\"^(\\\\([0-9]{3}\\\\))?[0-9]{3}-[0-"
        "9]{4}$\"}");
    SchemaDocument s(sd);
...
{
    Document sd;
    sd.Parse("{\"type\":\"string\",\"pattern\":\"a{0}\"}"); // TODO: report
                                                            // regex is invalid
                                                            // somehow
    SchemaDocument s(sd);
...
TEST(SchemaValidator, Integer_Range64Boundary)
{
    Document sd;
    sd.Parse(
        "{\"type\":\"integer\",\"minimum\":-9223372036854775807,\"maximum\":"
        "9223372036854775806}");
    SchemaDocument s(sd);
    ...
    INVALIDATE(s, "-9223372036854775808", "", "minimum", "",
               "{ \"minimum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": -9223372036854775807, \"actual\": "
               "-9223372036854775808"
               "}}");
    ...
    INVALIDATE(s, "18446744073709551615", "", "maximum", "",
               "{ \"maximum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": 9223372036854775806, \"actual\": "
...
TEST(SchemaValidator, Integer_RangeU64Boundary)
{
    ...
    INVALIDATE(s, "-9223372036854775808", "", "minimum", "",
               "{ \"minimum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": 9223372036854775808, \"actual\": "
               "-9223372036854775808"
               "}}");
    INVALIDATE(
        s, "9223372036854775807", "", "minimum", "",
        "{ \"minimum\": {"
        "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
    ...
    INVALIDATE(s, "18446744073709551615", "", "maximum", "",
               "{ \"maximum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": 18446744073709551614, \"actual\": "
               "18446744073709551615"
               "}}");
}
...
TEST(SchemaValidator, Integer_Range64BoundaryExclusive)
{
    ...
    sd.Parse(
        "{\"type\":\"integer\",\"minimum\":-9223372036854775808,\"maximum\":"
        "18446744073709551615,\"exclusiveMinimum\":true,\"exclusiveMaximum\":"
        "true}");
    SchemaDocument s(sd);

    INVALIDATE(
        s, "-9223372036854775808", "", "minimum", "",
        "{ \"minimum\": {"
        "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
    ...
    VALIDATE(s, "18446744073709551614", true);
    INVALIDATE(
        s, "18446744073709551615", "", "maximum", "",
        "{ \"maximum\": {"
        "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
...
TEST(SchemaValidator, Integer_MultipleOf64Boundary)
{
    ...
    INVALIDATE(s, "18446744073709551614", "", "multipleOf", "",
               "{ \"multipleOf\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": 18446744073709551615, \"actual\": "
               "18446744073709551614"
               "}}");
}
...
TEST(SchemaValidator, Number_RangeInt)
{
    Document sd;
    sd.Parse(
        "{\"type\":\"number\",\"minimum\":-100,\"maximum\":-1,"
        "\"exclusiveMaximum\":true}");
    SchemaDocument s(sd);
    ...
    INVALIDATE(s, "2147483647", "", "maximum", "",
               "{ \"maximum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": -1, \"exclusiveMaximum\": true, \"actual\": "
               "2147483647"
               "}}"); // int max
...
TEST(SchemaValidator, Number_RangeDouble)
{
    Document sd;
    sd.Parse(
        "{\"type\":\"number\",\"minimum\":0.1,\"maximum\":100.1,"
        "\"exclusiveMaximum\":true}");
    SchemaDocument s(sd);
    ...
    INVALIDATE(s, "18446744073709551614", "", "maximum", "",
               "{ \"maximum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": 100.1, \"exclusiveMaximum\": true, "
...
TEST(SchemaValidator, Number_RangeDoubleU64Boundary)
{
    Document sd;
    sd.Parse(
        "{\"type\":\"number\",\"minimum\":9223372036854775808.0,\"maximum\":"
    ...
    INVALIDATE(s, "-9223372036854775808", "", "minimum", "",
               "{ \"minimum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": 9223372036854775808.0, \"actual\": "
               "-9223372036854775808"
               "}}");
    INVALIDATE(
        s, "-2147483648", "", "minimum", "",
        "{ \"minimum\": {"
        "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
    ...
    INVALIDATE(s, "18446744073709551615", "", "maximum", "",
               "{ \"maximum\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
               "    \"expected\": 18446744073709550000.0, \"actual\": "
               "18446744073709551615"
               "}}");
}
...
TEST(SchemaValidator, Object)
{
    ...
    VALIDATE(s,
             "{\"Sun\":1.9891e30,\"Jupiter\":1.8986e27,\"Saturn\":5.6846e26,"
             "\"Neptune\":10.243e25,\"Uranus\":8.6810e25,\"Earth\":5.9736e24,"
             "\"Venus\":4.8685e24,\"Mars\":6.4185e23,\"Mercury\":3.3022e23,"
             "\"Moon\":7.349e22,\"Pluto\":1.25e22}",
             true);
    INVALIDATE(s, "[\"An\", \"array\", \"not\", \"an\", \"object\"]", "",
               "type", "",
               "{ \"type\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
...
TEST(SchemaValidator, Object_Properties)
{
    ...
    INVALIDATE(s,
               ...
               "{ \"type\": {"
               "    \"instanceRef\": \"#/number\", \"schemaRef\": "
               "\"#/properties/number\","
               "    \"expected\": [\"number\"], \"actual\": \"string\""
               "}}");
    INVALIDATE(s,
               "{ \"number\": \"One\", \"street_name\": \"Microsoft\", "
               "\"street_type\": \"Way\" }",
               "/properties/number", "type", "/number",
               "{ \"type\": {"
               "    \"instanceRef\": \"#/number\", \"schemaRef\": "
...
TEST(SchemaValidator, Object_AdditionalPropertiesObject)
{
    ...
    INVALIDATE(s,
               "{ \"number\": 1600, \"street_name\": \"Pennsylvania\", "
               "\"street_type\": \"Avenue\", \"office_number\": 201 }",
               "/additionalProperties", "type", "/office_number",
               "{ \"type\": {"
               "    \"instanceRef\": \"#/office_number\", \"schemaRef\": "
               "\"#/additionalProperties\","
               "    \"expected\": [\"string\"], \"actual\": \"integer\""
               "}}");
}
...
TEST(SchemaValidator, Object_Required)
{
    ...
    VALIDATE(s,
             "{ \"name\": \"William Shakespeare\", \"email\" : "
             "\"bill@stratford-upon-avon.co.uk\" }",
             true);
    VALIDATE(s,
             "{ \"name\": \"William Shakespeare\", \"email\" : "
             "\"bill@stratford-upon-avon.co.uk\", \"address\" : \"Henley "
             "Street, Stratford-upon-Avon, Warwickshire, England\", "
             "\"authorship\" : \"in question\"}",
             true);
    INVALIDATE(s,
               "{ \"name\": \"William Shakespeare\", \"address\" : \"Henley "
               "Street, Stratford-upon-Avon, Warwickshire, England\" }",
               "", "required", "",
               "{ \"required\": {"
               "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
...
TEST(SchemaValidator, Object_Required_PassWithDefault)
{
    ...
    sd.Parse(
        ...
        "    \"type\": \"object\","
        "    \"properties\" : {"
        "        \"name\":      { \"type\": \"string\", \"default\": \"William "
        "Shakespeare\" },"
    ...
    VALIDATE(s,
             "{ \"email\" : \"bill@stratford-upon-avon.co.uk\", \"address\" : "
...
TEST(SchemaValidator, Object_PropertyDependencies)
{
    ...
    VALIDATE(s,
             ...
             true);
    INVALIDATE(
        s, "{ \"name\": \"John Doe\", \"credit_card\": 5555555555555555 }", "",
        "dependencies", "",
        "{ \"dependencies\": {"
        "    \"instanceRef\": \"#\", \"schemaRef\": \"#\","
...
TEST(SchemaValidator, Object_SchemaDependencies)
{
    ...
    VALIDATE(s,
             "{\"name\": \"John Doe\", \"credit_card\" : "
             "5555555555555555,\"billing_address\" : \"555 Debtor's Lane\"}",
             true);
    INVALIDATE(s,
               "{\"name\": \"John Doe\", \"credit_card\" : 5555555555555555 }",
               "", "dependencies", "",
...
TEST(SchemaValidator, Object_PatternProperties)
{
    ...
    INVALIDATE(s, "{ \"S_0\": 42 }", "", "patternProperties", "/S_0",
               "{ \"type\": {"
               "    \"instanceRef\": \"#/S_0\", \"schemaRef\": "
               "\"#/patternProperties/%5ES_\","
               "    \"expected\": [\"string\"], \"actual\": \"integer\""
               "}}");
    INVALIDATE(s, "{ \"I_42\": \"This is a string\" }", "", "patternProperties",
               "/I_42",
               "{ \"type\": {"
               "    \"instanceRef\": \"#/I_42\", \"schemaRef\": "
               "\"#/patternProperties/%5EI_\","
               "    \"expected\": [\"integer\"], \"actual\": \"string\""
               "}}");
...
TEST(SchemaValidator, Object_PattternProperties_ErrorConflict)
{
    ...
    INVALIDATE(s, "{ \"I_30\": 7 }", "", "patternProperties", "/I_30",
               "{ \"multipleOf\": ["
               "    {"
               "      \"instanceRef\": \"#/I_30\", \"schemaRef\": "
               "\"#/patternProperties/%5EI_\","
               "      \"expected\": 5, \"actual\": 7"
               "    }, {"
               "      \"instanceRef\": \"#/I_30\", \"schemaRef\": "
               "\"#/patternProperties/30%24\","
               "      \"expected\": 6, \"actual\": 7"
               "    }"
               "]}");
}
...
TEST(SchemaValidator, Object_Properties_PatternProperties)
{
    ...
    INVALIDATE(
        s, "{ \"I_42\": 42 }", "", "patternProperties", "/I_42",
        "{ \"minimum\": {"
        "    \"instanceRef\": \"#/I_42\", \"schemaRef\": \"#/properties/I_42\","
        "    \"expected\": 73, \"actual\": 42"
        "}}");
    INVALIDATE(
        s, "{ \"I_42\": 7 }", "", "patternProperties", "/I_42",
        "{ \"minimum\": {"
        "    \"instanceRef\": \"#/I_42\", \"schemaRef\": \"#/properties/I_42\","
...
TEST(SchemaValidator, Object_PatternProperties_AdditionalProperties)
{
    ...
    VALIDATE(s, "{ \"keyword\": \"value\" }", true);
    INVALIDATE(s, "{ \"keyword\": 42 }", "/additionalProperties", "type",
               "/keyword",
               "{ \"type\": {"
               "    \"instanceRef\": \"#/keyword\", \"schemaRef\": "
               "\"#/additionalProperties\","
               "    \"expected\": [\"string\"], \"actual\": \"integer\""
               "}}");
}
...
TEST(SchemaValidator, Array_ItemsTuple)
{
    ...
    INVALIDATE(s, "[\"Palais de l'Elysee\"]", "/items/0", "type", "/0",
               ...
               "    \"expected\": [\"number\"], \"actual\": \"string\""
               "}}");
    INVALIDATE(s, "[\"Twenty-four\", \"Sussex\", \"Drive\"]", "/items/0",
               "type", "/0",
               "{ \"type\": {"
               "    \"instanceRef\": \"#/0\", \"schemaRef\": \"#/items/0\","
               "    \"expected\": [\"number\"], \"actual\": \"string\""
    ...
    VALIDATE(s, "[1600, \"Pennsylvania\", \"Avenue\", \"NW\", \"Washington\"]",
             true);
}
...
TEST(SchemaValidator, MultiTypeInObject)
{
    ...
    INVALIDATE(
        s, "{ \"tel\": true }", "/properties/tel", "type", "/tel",
        "{ \"type\": {"
...
TEST(SchemaValidator, MultiTypeWithObject)
{
    ...
    VALIDATE(s, "{ \"tel\": 999 }", true);
    INVALIDATE(
        s, "{ \"tel\": \"fail\" }", "/properties/tel", "type", "/tel",
        "{ \"type\": {"
        "    \"instanceRef\": \"#/tel\", \"schemaRef\": \"#/properties/tel\","
        "    \"expected\": [\"integer\"], \"actual\": \"string\""
        "}}");
...
TEST(SchemaValidator, EscapedPointer)
{
    ...
    SchemaDocument s(sd);
    INVALIDATE(
        s, "{\"~/\":true}", "/properties/~0~1", "type", "/~0~1",
        "{ \"type\": {"
...
class RemoteSchemaDocumentProvider
    : public IGenericRemoteSchemaDocumentProvider<SchemaDocumentType>
{
public:
        ...
        , schemaAllocator_(schemaBuffer_, sizeof(schemaBuffer_))
    {
        const char* filenames[kCount] = {
            ...
            "jsonschema/remotes/folder/folderInteger.json", "draft-04/schema"};
        ...
        for (size_t i = 0; i < kCount; i++)
        {
            ...
            else
            {
                ...
                MemoryPoolAllocator<> stackAllocator(stackBuffer,
                                                     sizeof(stackBuffer));
                DocumentType d(&documentAllocator_, 1024, &stackAllocator);
                d.Parse(json);
                sd_[i] = new SchemaDocumentType(d, uris[i],
    ...
    virtual const SchemaDocumentType* GetRemoteDocument(const char* uri,
                                                        SizeType length)
    {
        for (size_t i = 0; i < kCount; i++)
            if (typename SchemaDocumentType::URIType(uri, length) ==
                sd_[i]->GetURI())
                return sd_[i];
        return 0;
    }
...
private:
    typedef GenericDocument<typename SchemaDocumentType::EncodingType,
                            MemoryPoolAllocator<>, MemoryPoolAllocator<>>
        DocumentType;

    RemoteSchemaDocumentProvider(const RemoteSchemaDocumentProvider&);
    RemoteSchemaDocumentProvider& operator=(
        const RemoteSchemaDocumentProvider&);
...
{
    const char* filenames[] = {"additionalItems.json",
    ...
    typedef GenericSchemaDocument<Value, MemoryPoolAllocator<>>
        SchemaDocumentType;
    ...
    MemoryPoolAllocator<> jsonAllocator(jsonBuffer, sizeof(jsonBuffer));
    MemoryPoolAllocator<> documentAllocator(documentBuffer,
                                            sizeof(documentBuffer));
    MemoryPoolAllocator<> documentStackAllocator(documentStackBuffer,
                                                 sizeof(documentStackBuffer));
    MemoryPoolAllocator<> schemaAllocator(schemaBuffer, sizeof(schemaBuffer));
    MemoryPoolAllocator<> validatorAllocator(validatorBuffer,
    ...
    for (size_t i = 0; i < sizeof(filenames) / sizeof(filenames[0]); i++)
    {
        ...
        {
            GenericDocument<UTF8<>, MemoryPoolAllocator<>,
                            MemoryPoolAllocator<>>
                d(&documentAllocator, 1024, &documentStackAllocator);
            d.Parse(json);
            if (d.HasParseError())
            {
            ...
            {
                for (Value::ConstValueIterator schemaItr = d.Begin();
                     schemaItr != d.End(); ++schemaItr)
                {
                    {
                        SchemaDocumentType schema((*schemaItr)["schema"],
                                                  filenames[i],
                                                  static_cast<SizeType>(
                                                      strlen(filenames[i])),
                                                  &provider, &schemaAllocator);
                        GenericSchemaValidator<SchemaDocumentType,
                                               BaseReaderHandler<UTF8<>>,
                                               MemoryPoolAllocator<>>
                            validator(schema, &validatorAllocator);
                        const char* description1 =
                            (*schemaItr)["description"].GetString();
                        const Value& tests = (*schemaItr)["tests"];
                        for (Value::ConstValueIterator testItr = tests.Begin();
                             testItr != tests.End(); ++testItr)
                        {
                            const char* description2 =
                                (*testItr)["description"].GetString();
                            if (!onlyRunDescription ||
                                strcmp(description2, onlyRunDescription) == 0)
                            {
                                ...
                                if (expected != actual)
                                    printf("Fail: %30s \"%s\" \"%s\"\n",
                                           filename, description1,
                                           description2);
                                else
                                    passCount++;
                            }
                        }
                        // printf("%zu %zu %zu\n", documentAllocator.Size(),
                        // schemaAllocator.Size(), validatorAllocator.Size());
                    }
...
TEST(SchemaValidatingReader, Simple)
{
    ...
    Document d;
    StringStream ss("\"red\"");
    SchemaValidatingReader<kParseDefaultFlags, StringStream, UTF8<>> reader(ss,
                                                                            s);
    d.Populate(reader);
...
TEST(SchemaValidatingReader, Invalid)
{
    ...
    EXPECT_TRUE(reader.GetInvalidSchemaPointer() ==
                SchemaDocument::PointerType(""));
    EXPECT_TRUE(reader.GetInvalidDocumentPointer() ==
                SchemaDocument::PointerType(""));
...
TEST(SchemaValidatingWriter, Simple)
{
    ...
    GenericSchemaValidator<SchemaDocument, Writer<StringBuffer>> validator(
        s, writer);
    ...
    EXPECT_FALSE(d.Accept(validator));
    EXPECT_FALSE(validator.IsValid());
    EXPECT_TRUE(validator.GetInvalidSchemaPointer() ==
                SchemaDocument::PointerType(""));
    EXPECT_TRUE(validator.GetInvalidDocumentPointer() ==
                SchemaDocument::PointerType(""));
    Document e;
    e.Parse(
        "{ \"maxLength\": {"
...
TEST(SchemaValidator, Issue1017_allOfHandler)
{
    Document sd;
    sd.Parse(
        "{\"allOf\": [{\"type\": \"object\",\"properties\": {\"cyanArray2\": "
        "{\"type\": \"array\",\"items\": { \"type\": \"string\" "
        "}}}},{\"type\": \"object\",\"properties\": {\"blackArray\": "
        "{\"type\": \"array\",\"items\": { \"type\": \"string\" "
        "}}},\"required\": [ \"blackArray\" ]}]}");
    SchemaDocument s(sd);
    StringBuffer sb;
...
TEST(SchemaValidator, Ref_remote)
{
    ...
    typedef GenericSchemaValidator<
        SchemaDocumentType, BaseReaderHandler<UTF8<>>, MemoryPoolAllocator<>>
        SchemaValidatorType;
...
TEST(SchemaValidator, Ref_remote_issue1210)
{
    ...
    public:
        ...
        {
            int i = 0;
            while (collection[i] && SchemaDocument::URIType(uri, length) !=
                                        collection[i]->GetURI())
                ++i;
            return collection[i];
        }
    ...
    Document x, y, z;
    x.Parse(
        "{\"properties\":{\"country\":{\"$ref\":\"y.json#/definitions/"
        "country_remote\"}},\"type\":\"object\"}");
    y.Parse(
        "{\"definitions\":{\"country_remote\":{\"$ref\":\"z.json#/definitions/"
        "country_list\"}}}");
