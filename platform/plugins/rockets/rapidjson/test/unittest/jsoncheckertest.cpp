...
{
    const char* paths[] = {"jsonchecker", "bin/jsonchecker",
...
TEST(JsonChecker, Reader)
{
    ...
    for (int i = 1; i <= 33; i++)
    {
        if (i == 1) // fail1.json is valid in rapidjson, which has no limitation
                    // on type of root element (RFC 7159).
            continue;
        if (i == 18) // fail18.json is valid in rapidjson, which has no
                     // limitation on depth of nesting.
            continue;
        ...
        GenericDocument<UTF8<>, CrtAllocator> document; // Use Crt allocator to
                                                        // check
                                                        // exception-safety (no
                                                        // memory leak)
        document.Parse(json);
