...
be = BioExplorer('r1i6n34.bbp.epfl.ch:5000')
...
def add_virus(name, position, open_conformation_indices=list()):
    ...
    if add_glycans:
        ...
        be.add_multiple_glycans(
            ...
            indices=[17, 74, 149, 165, 282, 331, 343, 616, 1098, 1134, 1158, 1173, 1194], add_sticks=glycan_add_sticks,
            allowed_occurrences=closed_conformation_indices)
        be.add_multiple_glycans(
            assembly_name=name, glycan_type=be.NAME_GLYCAN_COMPLEX,
            protein_name=be.NAME_PROTEIN_S_OPEN, paths=complex_paths,
            indices=[17, 74, 149, 165, 282, 331, 343, 1098, 1134, 1158, 1173, 1194], index_offset=19,
    ...
    for i in range(nb_defensins):
        defensin = AssemblyProtein(
            ...
            source=defensins_path, assembly_params=Vector2(2.0, 0.0), shape=be.ASSEMBLY_SHAPE_SPHERICAL,
...
def add_cell(name, size, height, position=Vector3()):
    ...
    if add_glycans:
        ...
        for index in indices:
            ...
            o_glycan = Sugars(
                assembly_name=name, name=o_glycan_name, source=o_glycan_paths[0],
                protein_name=name + '_' + be.NAME_RECEPTOR, add_sticks=glycan_add_sticks, chain_ids=[2, 4],
                site_indices=[index[0]], orientation=index[1])
            be.add_glucoses(o_glycan)
...
def add_lymphocyte(name):
    lymphocyte = Mesh(source=lymphocyte_path)
    be.add_mesh(name=name, mesh=lymphocyte)
...
def test_low_glucose():
    ...
    add_virus(name='Coronavirus 1', position=Vector3(-5.0, 19.0, -36.0))
    add_virus(name='Coronavirus 2', position=Vector3(73.0, 93.0, -115.0))
    add_virus(name='Coronavirus 3', position=Vector3(-84.0, 110.0, 75.0))
    add_virus(name='Coronavirus 4', position=Vector3(-70.0, -100.0, 230.0), open_conformation_indices=[1])
    add_virus(name='Coronavirus 5', position=Vector3(200.0, 20.0, -150.0))

    add_cell(name='Cell 1', size=cell_size, height=cell_height, position=Vector3(4.5, -186, 7.0))

    add_surfactant_d(name='Surfactant-D 1', position=Vector3(74.0, 24.0, -45.0), random_seed=1)
    add_surfactant_d(name='Surfactant-D 2', position=Vector3(-30.0, 91.0, 20.0), random_seed=2)
    add_surfactant_d(name='Surfactant-D 3', position=Vector3(-165.0, 140.0, 105.0), random_seed=1)
    add_surfactant_d(name='Surfactant-D 4', position=Vector3(-260.0, 50.0, 0.0), random_seed=6)

    add_surfactant_a(name='Surfactant-A 1', position=Vector3(200.0, 50.0, 150.0), random_seed=2)

    add_glucose(cell_size, 120000)
    add_lactoferrins(cell_size, 150)
    add_defensins(cell_size, 300)
    add_lymphocyte(name='Emile')
    models = be.core_api().scene.models
    ...
    be.core_api().update_model(id=lymphocyte_model_id, transformation=transformation)
