...
{
    PLUGIN_INFO << "Registering 'field_renderer' renderer" << std::endl;
    brayns::PropertyMap properties;
    properties.setProperty({"exposure", 1., 1., 10., {"Exposure"}});
    properties.setProperty(
        {"nbRefinementSteps", 8, 1, 128, {"Number of refinement steps"}});
    properties.setProperty(
        {"nbRaySteps", 64, 2, 2048, {"Number of ray marching steps"}});
    properties.setProperty({"cutoff", 300.0, 0.001, 1e5, {"Cutoff distance"}});
    properties.setProperty(
        {"spaceDistortion", 0.0, 0.0, 10.0, {"Space distortion"}});
    properties.setProperty(
        {"alphaCorrection", 0.05, 0.001, 1.0, {"Alpha correction"}});
    properties.setProperty(
        {"renderDirections", false, {"Render field directions"}});
    properties.setProperty(
        {"processGeometry", false, {"Include geometry (slow)"}});
...
{
    PLUGIN_INFO << "Registering 'octree_field_renderer' renderer" << std::endl;
    brayns::PropertyMap properties;
    properties.setProperty({"exposure", 1., 1., 10., {"Exposure"}});
    properties.setProperty(
...
FieldRendererPlugin::FieldRendererPlugin(const std::string &connectionString,
                                         const std::string &schema,
                                         const bool useOctree,
                                         const bool loadGeometry,
...
void FieldRendererPlugin::init()
{
    ...
    if (actionInterface)
    {
        ...
        PLUGIN_INFO << "Registering '" + entryPoint + "' endpoint" << std::endl;
        actionInterface->registerRequest<SimulationParameters, Response>(
            entryPoint, [&](const SimulationParameters &payload) {
    ...
    if (_useOctree)
    {
        ...
        _api->getParametersManager()
            ...
            .setCurrentRenderer("octree_field_renderer");
    }
    else
    {
        ...
        _api->getParametersManager()
            .getRenderingParameters()
            .setCurrentRenderer("field_renderer");
    }
...
void FieldRendererPlugin::_attachFieldsHandler()
{
    ...
    if (_useOctree)
    {
        OctreeFieldHandlerPtr handler =
            std::make_shared<OctreeFieldHandler>(_uri, _schema,
                                                 _useCompartments);
        model->setSimulationHandler(handler);
    }
    else
    {
        FieldHandlerPtr handler =
            std::make_shared<FieldHandler>(_uri, _schema, _useCompartments);
        model->setSimulationHandler(handler);
    }
    ...
    else
    {
        ...
        model->addSphere(materialId,
                         {box.getCenter(),
                          std::max(halfSize.x,
                                   std::max(halfSize.y, halfSize.z))});
    }

    auto modelDescriptor =
        std::make_shared<ModelDescriptor>(std::move(model), "Field gizmo");
    scene.addModel(modelDescriptor);
}

Response FieldRendererPlugin::_setSimulationParameters(
    const SimulationParameters &payload)
{
    ...
    try
    {
        if (payload.density > 100)
            PLUGIN_THROW(
                std::runtime_error("Density cannot be more than 100%"));
        ...
        if (modelDescriptor)
        {
            ...
            if (!handler)
                PLUGIN_THROW(
                    std::runtime_error("Model has no simulation handler"));

            if (_useOctree)
            {
                OctreeFieldHandler *fieldHandler =
                    dynamic_cast<OctreeFieldHandler *>(handler.get());
                if (!fieldHandler)
                    PLUGIN_THROW(std::runtime_error(
                        "Model simulation handler is not an octree-based field "
                        "simulation handler"));
            ...
            else
            {
                FieldHandler *fieldHandler =
                    dynamic_cast<FieldHandler *>(handler.get());
                if (!fieldHandler)
                    PLUGIN_THROW(
...
extern "C" ExtensionPlugin *brayns_plugin_create(int argc, char **argv)
{
    ...
    PLUGIN_INFO << "_|        _|    _|_|_|  _|    _|_|_|  _|_|_|    "
                << std::endl;
    PLUGIN_INFO << std::endl;
    PLUGIN_INFO << "Initializing Fields plug-in (version " << PLUGIN_VERSION
                << ")" << std::endl;
    PLUGIN_INFO << std::endl;
    ...
    PLUGIN_INFO << "URI: " << uri << ", Schema: " << schema << std::endl;

    return new FieldRendererPlugin(uri, schema, useOctree, loadGeometry,
                                   useCompartments);
}
