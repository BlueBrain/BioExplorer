...
{
OctreeFieldHandler::OctreeFieldHandler(const std::string& uri,
...
void OctreeFieldHandler::_buildOctree(const size_t frame)
{
    ...
    const float voxelSize =
        std::max(sceneSize.x, std::max(sceneSize.y, sceneSize.z)) / 4096.f;
    ...
    PLUGIN_INFO << "--------------------------------------------" << std::endl;
    PLUGIN_INFO << "Scene AABB        : [" << box << "]" << std::endl;
    PLUGIN_INFO << "Scene dimension   : [" << sceneSize.x << "," << sceneSize.y
                << "," << sceneSize.z << "]" << std::endl;
    PLUGIN_INFO << "Voxel size        : [" << voxelSize << "] " << std::endl;
    ...
    if (_frameSize > 1073741824)
        PLUGIN_ERROR << "Octree size exceeds 32 bits capacity" << std::endl;
}
