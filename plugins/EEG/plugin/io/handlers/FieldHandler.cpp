    ...
    : brayns::AbstractSimulationHandler()
{
    ...
    for (size_t c; c < 100; ++c)
    {
        ...
        _cells.push_back(
            {{rand() % 1000 / 100.f - 5.f, rand() % 1000 / 100.f - 5.f,
              rand() % 1000 / 100.f - 5.f},
             values});
    }
...
FieldHandler::FieldHandler(const std::string& uri, const std::string& schema,
...
void* FieldHandler::getFrameData(const uint32_t frame)
{
    ...
    else
    {
        ...
        const auto& points =
            _connector->getSpikingNeurons(frame * _dt, _params.decaySteps * _dt,
                                          _params.density);
    ...
    PLUGIN_DEBUG << "Simulation data size: " << _frameData.size() << std::endl;
    return _frameData.data();
}

void FieldHandler::importFromFile(const std::string& filename)
{
    PLUGIN_INFO << "Loading data from file: " << filename << std::endl;
    ...
    PLUGIN_INFO << "Loading " << _frameSize << " cell, and " << _nbFrames
                << " frames" << std::endl;
