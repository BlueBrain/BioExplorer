...
void OctreeFieldRenderer::commit()
{
    ...
    if (transferFunction)
        ispc::OctreeFieldRenderer_setTransferFunction(
            getIE(), transferFunction->getIE());

    // Renderer
    ispc::OctreeFieldRenderer_set(
        getIE(), (_userData ? (float*)_userData->data : nullptr), _userDataSize,
        spp, _lightPtr, _lightArray.size(), _minRayStep, _nbRaySteps,
        _nbRayRefinementSteps, _exposure, _cutoff, _alphaCorrection);
}
