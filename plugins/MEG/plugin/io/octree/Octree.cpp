...
Octree::Octree(const std::vector<float> &events, float voxelSize, const glm::vec3 &minAABB, const glm::vec3 &maxAABB)
    ...
    , _offsetPerLevel(nullptr)
{
    ...
    // **************** Octree creations *******************
    // *****************************************************
    glm::uvec3 octreeSize(_pow2roundup(std::ceil((maxAABB.x - minAABB.x) / voxelSize)),
    ...
    for (uint32_t i = 0; i < events.size(); i += 5)
    {
        ...
        if (it == octree[0].end())
        {
            ...
            for (uint32_t level = 0; level < octreeDepth; ++level)
            {
                ...
                const glm::vec3 center(divisor * (xpos / divisor + 0.5f), divisor * (ypos / divisor + 0.5f),
                                       divisor * (zpos / divisor + 0.5f));

                const uint64_t nBlock = _octreeSize / divisor;
                const uint64_t index = std::floor(xpos / divisor) + nBlock * std::floor(ypos / divisor) +
    ...
    _volumeDim =
        glm::uvec3(std::ceil((maxAABB.x - minAABB.x) / voxelSize), std::ceil((maxAABB.y - minAABB.y) / voxelSize),
                   std::ceil((maxAABB.z - minAABB.z) / voxelSize));
    _volumeSize = (uint64_t)_volumeDim.x * (uint64_t)_volumeDim.y * (uint64_t)_volumeDim.z;
}
