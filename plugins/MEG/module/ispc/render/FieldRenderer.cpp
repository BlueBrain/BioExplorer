...
void FieldRenderer::commit()
{
    ...
    if (transferFunction)
        ispc::FieldRenderer_setTransferFunction(getIE(), transferFunction->getIE());
    ...
    // Renderer
    ispc::FieldRenderer_set(getIE(), (_userData ? (float*)_userData->data : nullptr), _userDataSize, _randomNumber,
                            _timestamp, spp, _lightPtr, _lightArray.size(), _nbRaySteps, _nbRefinementSteps, _exposure,
                            _cutoff, _alphaCorrection, _renderDirections, _spaceDistortion, _processGeometry,
                            _normalized);
}
