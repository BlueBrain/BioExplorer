...
TEST(Value, MoveConstructor)
{
    ...
    x.PushBack(1, allocator)
        .PushBack(2, allocator)
        .PushBack(3, allocator)
...
TEST(Value, Uint)
{
    ...
    EXPECT_TRUE(x.IsInt64());
    EXPECT_TRUE(x.IsUint64());
    EXPECT_NEAR(1234.0, x.GetDouble(),
                0.0); // Number can always be cast as double but !IsDouble().
    ...
    EXPECT_TRUE(z.IsInt64()); // Issue 41: Incorrect parsing of unsigned int
                              // number types
...
TEST(Value, IsLosslessDouble)
{
    ...
    EXPECT_TRUE(Value(-static_cast<int64_t>(
                          RAPIDJSON_UINT64_C2(0x40000000, 0x00000000)))
                    .IsLosslessDouble());
#if !(defined(_MSC_VER) && _MSC_VER < 1800) // VC2010 has problem
    EXPECT_TRUE(
        Value(RAPIDJSON_UINT64_C2(0xA0000000, 0x00000000)).IsLosslessDouble());
#endif

    EXPECT_FALSE(
        Value(static_cast<int64_t>(RAPIDJSON_UINT64_C2(0x7FFFFFFF, 0xFFFFFFFF)))
            .IsLosslessDouble()); // INT64_MAX
    EXPECT_FALSE(Value(-static_cast<int64_t>(
                           RAPIDJSON_UINT64_C2(0x7FFFFFFF, 0xFFFFFFFF)))
                     .IsLosslessDouble()); // -INT64_MAX
    EXPECT_TRUE(Value(-static_cast<int64_t>(
                          RAPIDJSON_UINT64_C2(0x7FFFFFFF, 0xFFFFFFFF)) -
                      1)
                    .IsLosslessDouble()); // INT64_MIN
    EXPECT_FALSE(Value(RAPIDJSON_UINT64_C2(0xFFFFFFFF, 0xFFFFFFFF))
                     .IsLosslessDouble()); // UINT64_MAX
...
TEST(Value, String)
{
    ...
    Value y(kStringType);
    EXPECT_TRUE(y.IsString());
    EXPECT_STREQ("", y.GetString()); // Empty string should be "" instead of 0
                                     // (issue 226)
...
static void TestArray(T& x, Allocator& allocator)
{
    ...
    // PushBack(GenericValue&&, Allocator&);
    {
        ...
        y2.PushBack(std::move(Value(kArrayType)
                                  .PushBack(Value(1), allocator)
                                  .PushBack("foo", allocator)),
                    allocator);
    ...
    for (unsigned first = 0; first < n; first++)
    {
        for (unsigned last = first; last <= n; last++)
        {
            ...
            for (unsigned i = 0; i < n; i++)
                x.PushBack(Value(kArrayType).PushBack(i, allocator).Move(),
                           allocator);
            ...
            for (unsigned i = first; i < n - removeCount; i++)
                EXPECT_EQ(i + removeCount,
                          x[static_cast<SizeType>(i)][0].GetUint());
        }
    }
}
...
TEST(Value, ArrayHelper)
{
...

    {
        ...
        Value y(kArrayType);
        y.PushBack(123, allocator);
        x.PushBack(y.GetArray(), allocator); // Implicit constructor to convert
                                             // Array to GenericValue
...
static void TestObject(T& x, Allocator& allocator)
{
    ...
    // AddMember(GenericValue&&, ...) variants
    {
        ...
        o.AddMember(Value("false"), Value(false).Move(),
                    allocator); // value is lvalue ref
        o.AddMember(Value("int").Move(), Value(-1),
                    allocator); // name is lvalue ref
        o.AddMember("uint", std::move(Value().SetUint(1u)),
                    allocator); // name is literal, value is rvalue
        EXPECT_TRUE(o["true"].GetBool());
        EXPECT_FALSE(o["false"].GetBool());
        EXPECT_EQ(-1, o["int"].GetInt());
        EXPECT_EQ(1u, o["uint"].GetUint());
    ...
    for (int i = 0; i < 10; i++)
        x.AddMember(keys[i], Value(kArrayType).PushBack(i, allocator),
                    allocator);
    ...
    for (unsigned first = 0; first < n; first++)
    {
        for (unsigned last = first; last <= n; last++)
        {
            ...
            itr = x.EraseMember(x.MemberBegin() + static_cast<int>(first),
                                x.MemberBegin() + static_cast<int>(last));
            if (last == n)
                EXPECT_EQ(x.MemberEnd(), itr);
            else
            ...
            for (unsigned i = first; i < n - removeCount; i++)
                EXPECT_EQ(i + removeCount,
                          x[keys[i + removeCount]][0].GetUint());
        }
    }
...
TEST(Document, CrtAllocator)
{
    ...
    V o(kObjectType);
    o.AddMember("x", 1,
                allocator); // Should not call destructor on uninitialized
                            // name/value of newly allocated members.

    V a(kArrayType);
    a.PushBack(1, allocator); // Should not call destructor on uninitialized
                              // Value of newly allocated elements.
}

static void TestShortStringOptimization(const char* str)
{
    const rapidjson::SizeType len =
        static_cast<rapidjson::SizeType>(strlen(str));
...
TEST(Value, AllocateShortString)
{
    ...
    TestShortStringOptimization("12345678"); // regular case for short strings:
                                             // 8 chars
    TestShortStringOptimization("12345678901"); // edge case: 11 chars in 32-bit
                                                // mode (=> short string)
    TestShortStringOptimization("123456789012");     // edge case: 12 chars in
                                                     // 32-bit mode (=> regular
                                                     // string)
    TestShortStringOptimization("123456789012345");  // edge case: 15 chars in
                                                     // 64-bit mode (=> short
                                                     // string)
    TestShortStringOptimization("1234567890123456"); // edge case: 16 chars in
                                                     // 64-bit mode (=> regular
                                                     // string)
}
