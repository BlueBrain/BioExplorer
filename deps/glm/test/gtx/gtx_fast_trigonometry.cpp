...
int perf(bool NextFloat)
{
    ...
    for (float i = begin; i < end;
         i = NextFloat ? glm::next_float(i) : i += 0.1f)
...
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> fastCosNew(glm::vec<L, T, Q> const& x)
{
    ...
    /*
            vec<L, bool, Q> const FirstQuarterPi(lessThanEqual(Angle0_PI, vec<L,
       T, Q>(glm::half_pi<T>())));

            vec<L, T, Q> const RevertAngle(mix(vec<L, T, Q>(glm::pi<T>()),
       vec<L, T, Q>(0), FirstQuarterPi)); vec<L, T, Q> const
       ReturnSign(mix(vec<L, T, Q>(-1), vec<L, T, Q>(1), FirstQuarterPi));
    ...
    glm::vec<L, T, Q> const& x)
{
    ...
    glm::vec<L, T, Q> const RevertAngle(mix(glm::vec<L, T, Q>(glm::pi<T>()),
                                            glm::vec<L, T, Q>(0),
                                            FirstQuarterPi));
...
int perf_fastCosDeterminisctic(float Begin, float End, std::size_t Samples)
{
    ...
    for (std::size_t i = 0; i < Samples; ++i)
        Results[i] = taylorCos::fastCosDeterminisctic(
            AngleShift + glm::vec4(Begin + Steps * i));
...
int perf_fastCosRef(float Begin, float End, std::size_t Samples)
{
    ...
    for (std::size_t i = 0; i < Samples; ++i)
        Results[i] =
            taylorCos::fastRefCos(AngleShift + glm::vec4(Begin + Steps * i));
...
int test()
{
    ...
    // glm::pi<float>(); Angle += 0.1f) for(float Angle = -720.0f; Angle <
    // 720.0f; Angle += 0.1f)
