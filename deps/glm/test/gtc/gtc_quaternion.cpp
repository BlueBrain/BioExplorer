...
int test_quat_angle()
{
    ...
    {
        glm::quat Q =
            glm::angleAxis(glm::pi<float>() * 0.25f, glm::vec3(0, 0, 1));
...
int test_quat_normalize()
{
    ...
    {
        glm::quat Q =
            glm::angleAxis(glm::pi<float>() * 0.25f, glm::vec3(0, 0, 2));
        glm::quat N = glm::normalize(Q);
        float L = glm::length(N);
        Error += glm::equal(L, 1.0f, 0.000001f) ? 0 : 1;
    }
    {
        glm::quat Q =
            glm::angleAxis(glm::pi<float>() * 0.25f, glm::vec3(1, 2, 3));
        glm::quat N = glm::normalize(Q);
        float L = glm::length(N);
        Error += glm::equal(L, 1.0f, 0.000001f) ? 0 : 1;
    }
...
int test_quat_slerp()
{
    ...
    // Testing almost equal quaternions (this test should pass through the
    // linear interpolation) Must be 0 0.00X 0 0.99999
    glm::quat almostid =
