    ...
    for (glm::uint16 w = 0; w < w_max; w += 27)
        for (glm::uint16 z = 0; z < z_max; z += 27)
            for (glm::uint16 y = 0; y < y_max; y += 27)
                ...
                {
                    glm::uint64 ResultA =
                        loopBitfieldInterleave<glm::uint16, glm::uint64>(x, y,
                                                                         z, w);
...
int test()
{
    ...
    #				if GLM_ARCH & GLM_ARCH_SSE2_BIT
                        ...
                        __m128i G = glm_i128_interleave(_mm_set_epi32(0, y, 0,
    x)); glm::uint64 Result[2]; _mm_storeu_si128((__m128i*)Result, G); assert(A
    == Result[0]); #				endif//GLM_ARCH & GLM_ARCH_SSE2_BIT
                }
            }
    */
    {
...

    {
        for (glm::uint8 y = 0; y < 127; ++y)
            ...
            {
                glm::int64 A(
                    glm::bitfieldInterleave(glm::int8(x), glm::int8(y)));
                glm::int64 B(
                    glm::bitfieldInterleave(glm::int16(x), glm::int16(y)));
                glm::int64 C(
                    glm::bitfieldInterleave(glm::int32(x), glm::int32(y)));
...
int perf()
{
    ...
    */
...

    {
        ...
        for (std::size_t i = 0; i < Data.size(); ++i)
            Data[i] =
                glm::bitfieldInterleave(Param[i].x, Param[i].y, Param[i].x);
        ...
        for (glm::size_t j = 0; j < 256; ++j)
            for (glm::size_t i = 0; i < 256; ++i)
                Error += Result[j * 256 + i] ==
                                 glm::bitfieldInterleave(glm::uint8(i),
                                                         glm::uint8(j))
                             ? 0
                             : 1;
...
int perf_new_u8vec2(std::vector<glm::uint16>& Result)
{
    ...
    for (glm::size_t k = 0; k < 10000; ++k)
        for (glm::size_t j = 0; j < 256; ++j)
            for (glm::size_t i = 0; i < 256; ++i)
                Error += Result[j * 256 + i] ==
                                 bitfieldInterleave_u8vec2(glm::uint8(i),
                                                           glm::uint8(j))
                             ? 0
                             : 1;
...
int perf_new_u8vec4(std::vector<glm::uint32>& Result)
{
    ...
    for (glm::size_t k = 0; k < 10000; ++k)
        for (glm::size_t j = 0; j < 256; ++j)
            for (glm::size_t i = 0; i < 256; ++i)
                Error += Result[j * 256 + i] ==
                                 bitfieldInterleave_u8vec4(glm::uint8(i),
                                                           glm::uint8(j),
                                                           glm::uint8(i),
                                                           glm::uint8(j))
                             ? 0
...
int perf_old_u16vec2(std::vector<glm::uint32>& Result)
{
    ...
    for (glm::size_t k = 0; k < 10000; ++k)
        for (glm::size_t j = 0; j < 256; ++j)
            for (glm::size_t i = 0; i < 256; ++i)
                Error += Result[j * 256 + i] ==
                                 glm::bitfieldInterleave(glm::uint16(i),
                                                         glm::uint16(j))
                             ? 0
                             : 1;
...
int perf_new_u16vec2(std::vector<glm::uint32>& Result)
{
    ...
    for (glm::size_t k = 0; k < 10000; ++k)
        for (glm::size_t j = 0; j < 256; ++j)
            for (glm::size_t i = 0; i < 256; ++i)
                Error += Result[j * 256 + i] ==
                                 bitfieldInterleave_u16vec2(glm::uint16(i),
                                                            glm::uint16(j))
                             ? 0
                             : 1;
