...
int test()
{
...

    {
        ...
        Error += glm::all(glm::epsilonEqual(ColorSourceRGB, ColorRGB, 0.00001f))
                     ? 0
                     : 1;
    }

    {
        glm::vec3 const ColorSRGB =
            glm::convertLinearToSRGB(ColorSourceRGB, 2.8f);
...

    {
        ...
        Error +=
            glm::all(glm::epsilonEqual(ColorSourceRGBA, ColorRGB, 0.00001f))
                ? 0
                : 1;
    }

    {
        glm::vec4 const ColorSRGB =
            glm::convertLinearToSRGB(ColorSourceRGBA, 2.8f);
...
int test()
{
    ...
    for (float Color = 0.0f; Color < 1.0f; Color += 0.01f)
    {
        ...
        Error += glm::all(glm::epsilonEqual(glm::abs(HighpSRGB -
                                                     glm::highp_vec3(LowpSRGB)),
