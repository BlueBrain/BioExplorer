...
int test_linearRand()
{
...

    {
        ...
        glm::u8vec2 AMax(std::numeric_limits<glm::u8>::min());
        {
            ...
            {
                glm::u8vec2 A =
                    glm::linearRand(glm::u8vec2(Min), glm::u8vec2(Max));
                AMin = glm::min(AMin, A);
                AMax = glm::max(AMax, A);
        ...
        glm::u16vec2 BMax(std::numeric_limits<glm::u16>::min());
        {
            ...
            {
                glm::u16vec2 B =
                    glm::linearRand(glm::u16vec2(Min), glm::u16vec2(Max));
                BMin = glm::min(BMin, B);
                BMax = glm::max(BMax, B);
        ...
        glm::u32vec2 CMax(std::numeric_limits<glm::u32>::min());
        {
            ...
            {
                glm::u32vec2 C =
                    glm::linearRand(glm::u32vec2(Min), glm::u32vec2(Max));
                CMin = glm::min(CMin, C);
                CMax = glm::max(CMax, C);
        ...
        glm::u64vec2 DMax(std::numeric_limits<glm::u64>::min());
        {
            ...
            {
                glm::u64vec2 D =
                    glm::linearRand(glm::u64vec2(Min), glm::u64vec2(Max));
                DMin = glm::min(DMin, D);
                DMax = glm::max(DMax, D);
...

    {
        ...
        glm::i8vec2 AMax(std::numeric_limits<glm::i8>::min());
        {
            ...
            {
                glm::i8vec2 A =
                    glm::linearRand(glm::i8vec2(Min), glm::i8vec2(Max));
                AMin = glm::min(AMin, A);
                AMax = glm::max(AMax, A);
        ...
        glm::i16vec2 BMax(std::numeric_limits<glm::i16>::min());
        {
            ...
            {
                glm::i16vec2 B =
                    glm::linearRand(glm::i16vec2(Min), glm::i16vec2(Max));
                BMin = glm::min(BMin, B);
                BMax = glm::max(BMax, B);
        ...
        glm::i32vec2 CMax(std::numeric_limits<glm::i32>::min());
        {
            ...
            {
                glm::i32vec2 C =
                    glm::linearRand(glm::i32vec2(Min), glm::i32vec2(Max));
                CMin = glm::min(CMin, C);
                CMax = glm::max(CMax, C);
        ...
        glm::i64vec2 DMax(std::numeric_limits<glm::i64>::min());
        {
            ...
            {
                glm::i64vec2 D =
                    glm::linearRand(glm::i64vec2(Min), glm::i64vec2(Max));
                DMin = glm::min(DMin, D);
                DMax = glm::max(DMax, D);
    ...
    for (std::size_t i = 0; i < TestSamples; ++i)
    {
        ...
        if (!glm::all(
                glm::lessThanEqual(A, glm::f32vec2(static_cast<float>(Max)))))
            ++Error;
        if (!glm::all(glm::greaterThanEqual(A, glm::f32vec2(
                                                   static_cast<float>(Min)))))
            ++Error;

        glm::f64vec2 const B(
            glm::linearRand(glm::f64vec2(Min), glm::f64vec2(Max)));
...
int test_circularRand()
{
...

    {
        ...
        Error +=
            glm::epsilonEqual(ResultDouble, double(Max) * double(Radius), 0.01)
                ? 0
                : 1;
        assert(!Error);
    }
...
int test_sphericalRand()
{
...

    {
        ...
        Error +=
            glm::epsilonEqual(ResultDoubleB, double(Max * 2), 0.0001) ? 0 : 1;
        Error += glm::epsilonEqual(ResultFloatC, float(Max * 3), 0.01f) ? 0 : 1;
        Error +=
            glm::epsilonEqual(ResultDoubleC, double(Max * 3), 0.01) ? 0 : 1;
        assert(!Error);
    }
