...
//	return _mm_shuffle_ps(Src, Src, mask<(int(W) << 6) | (int(Z) << 4) | (int(Y)
//<< 2) | (int(X) << 0)>::value);
//}
...
static int test_operators()
{
...

    {
        ...
        Error += glm::all(glm::equal(C, glm::vec4(5, 7, 9, 11),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;

        glm::vec4 const D = B - A;
        Error += glm::all(glm::equal(D, glm::vec4(3, 3, 3, 3),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;

        glm::vec4 const E = A * B;
        Error += glm::all(glm::equal(E, glm::vec4(4, 10, 18, 28),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;
        ...
        Error += glm::all(glm::equal(G, glm::vec4(2, 3, 4, 5),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;

        glm::vec4 const H = B - 1.0f;
        Error += glm::all(glm::equal(H, glm::vec4(3, 4, 5, 6),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;

        glm::vec4 const I = A * 2.0f;
        Error += glm::all(glm::equal(I, glm::vec4(2, 4, 6, 8),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;
        ...
        Error += glm::all(glm::equal(K, glm::vec4(2, 3, 4, 5),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;
        ...
        Error += glm::all(glm::equal(M, glm::vec4(2, 4, 6, 8),
                                     glm::epsilon<float>()))
                     ? 0
                     : 1;
        ...
        Error +=
            glm::all(glm::equal(N,
                                glm::vec4(0.5, 2.0 / 5.0, 2.0 / 6.0, 2.0 / 7.0),
                                glm::epsilon<float>()))
                ? 0
                : 1;
    }
