...
static int test()
{
    ...
    {
        glm::uint Return =
            glm::bitfieldInsert(Data32[i].Base, Data32[i].Insert,
...
static int test()
{
    ...
    {
        glm::uint Return =
            glm::bitfieldExtract(Data32[i].Value, Data32[i].Offset,
                                 Data32[i].Bits);
...
/*
    GLM_FUNC_QUALIFIER unsigned int bitfieldReverseLoop(unsigned int v)
    {
        ...
        unsigned int const BitSize = static_cast<unsigned int>(sizeof(unsigned
   int) * 8); for(unsigned int i = 0; i < BitSize; ++i)
        {
    ...
    GLM_FUNC_QUALIFIER glm::uint64_t bitfieldReverseLoop(glm::uint64_t v)
    {
        ...
        glm::uint64_t const BitSize = static_cast<glm::uint64_t>(sizeof(unsigned
   int) * 8); for(glm::uint64_t i = 0; i < BitSize; ++i)
        {
            glm::uint64_t const BitSet(v & (static_cast<glm::uint64_t>(1) <<
   i)); glm::uint64_t const BitFirst(BitSet >> i); Result |= BitFirst <<
   (BitSize - 1 - i);
        }
        return Result;
    }
...
typeU32 const Data32[] = {{0x00000001, 0x80000000, SUCCESS},
...
static int test_int()
{
    ...
    entry const Data[] = {{0x00000000, -1}, {0x00000001, 0},  {0x00000002, 1},
                          {0x00000003, 1},  {0x00000004, 2},  {0x00000005, 2},
                          {0x00000007, 2},  {0x00000008, 3},  {0x00000010, 4},
                          {0x00000020, 5},  {0x00000040, 6},  {0x00000080, 7},
                          {0x00000100, 8},  {0x00000200, 9},  {0x00000400, 10},
                          {0x00000800, 11}, {0x00001000, 12}, {0x00002000, 13},
                          {0x00004000, 14}, {0x00008000, 15}, {0x00010000, 16},
                          {0x00020000, 17}, {0x00040000, 18}, {0x00080000, 19},
                          {0x00100000, 20}, {0x00200000, 21}, {0x00400000, 22},
                          {0x00800000, 23}, {0x01000000, 24}, {0x02000000, 25},
                          {0x04000000, 26}, {0x08000000, 27}, {0x10000000, 28},
                          {0x20000000, 29}, {0x40000000, 30}};
...
entry const DataI32[] = {{0x00000001, 0},
                         {0x00000003, 0},
                         {0x00000002, 1},
                         // {0x80000000, 31}, // Clang generates an error with
                         // this
                         {0x00010000, 16},
                         {0x7FFF0000, 16},
                         {0x7F000000, 24},
...
static int findLSB_branchfree(genIUType x)
{
    ...
    return static_cast<int>(
               glm::bitCount(~x & (x - static_cast<genIUType>(1)))) *
               Keep +
           Discard * -1;
}
...
static glm::vec<L, int, Q> bitCount_bitfield(glm::vec<L, T, Q> const& v)
{
    glm::vec<L, typename glm::detail::make_unsigned<T>::type, Q> x(
        *reinterpret_cast<glm::vec<
            L, typename glm::detail::make_unsigned<T>::type, Q> const*>(&v));
