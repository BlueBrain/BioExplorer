...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const float text_base_offset_y =
        ImMax(0.0f,
              window->DC.CurrentLineTextBaseOffset); // Latch before ItemSize
                                                     // changes it
    ...
    const ImRect bb(
        ...
        window->DC.CursorPos +
            ImVec2(g.FontSize + (label_size.x > 0.0f
                                     ...
                                     : 0.0f),
                   ImMax(line_height, label_size.y))); // Empty text doesn't add
                                                       // padding
    ItemSize(bb);
    if (!ItemAdd(bb, 0))
        return;
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max,
                                    ...
                                    window_rounding_corners);
    bb.Expand(ImVec2(-ImClamp((float)(int)((bb.Max.x - bb.Min.x - 2.0f) * 0.5f),
                              0.0f, 3.0f),
...
void ImGui::AlignTextToFramePadding()
{
    ...
    window->DC.CurrentLineSize.y =
        ImMax(window->DC.CurrentLineSize.y,
              g.FontSize + g.Style.FramePadding.y * 2);
    window->DC.CurrentLineTextBaseOffset =
                       ...
                       ImGuiComboFlags flags)
{
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            ImVec2 pos =
                FindBestWindowPosForPopupEx(frame_bb.GetBL(), size_expected,
                       ...
                       const char* format, float power)
{
    ...
    if (power != 1.0f)
        IM_ASSERT(v_min != NULL && v_max != NULL); // When using a power curve
                                                   // the drag needs to have
                                                   // known bounds
                            ...
                            const char* format_max, float power)
{
    ...
    bool value_changed =
        DragFloat("##min", v_current_min, v_speed,
                  (v_min >= v_max) ? -FLT_MAX : v_min,
                  (v_min >= v_max) ? *v_current_max
                          ...
                          int v_max, const char* format, const char* format_max)
{
    ...
    bool value_changed =
        DragInt("##min", v_current_min, v_speed,
                            ...
                            ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (!is_decimal && v_range >= 0) // v_range < 0 may happen on integer
                                     // overflows
        grab_sz =
    ...
    float grab_t =
        SliderCalcRatioFromValueT<TYPE, FLOATTYPE>(data_type, *v, v_min, v_max,
                                           ...
                                           void* data_ptr, const char* format)
{
    ...
    bool value_changed = InputTextEx(label, data_buf, IM_ARRAYSIZE(data_buf),
                                     bb.GetSize(), flags);
    if (g.ScalarAsInputTextId == 0) // First frame we started displaying the
                                    // InputText widget
    {
        ...
        *out_offset =
            ImVec2(line_width,
                   text_size.y + line_height); // offset allow for the
                                               // possibility of sitting after a
                                               // trailing \n
                        ...
                        void* callback_user_data)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(),
                               (is_multiline ? GetTextLineHeight() * 8.0f
                                             : label_size.y) +
                                   style.FramePadding.y *
                                       2.0f); // Arbitrary default of 8 lines
                                              // high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(
        frame_bb.Min,
    ...
    if (is_multiline)
    {
        ...
        draw_window->DC.NavLayerActiveMaskNext |=
            draw_window->DC.NavLayerCurrentMask; // This is to ensure that
    ...
    const bool focus_requested =
        FocusableItemRegister(window, id,
                              (flags & (ImGuiInputTextFlags_CallbackCompletion |
                                  ...
                                  0); // Using completion callback disable
                                      // keyboard tabbing
        ...
        user_nav_input_start)
    {
        if (g.ActiveId != id)
        {
            ...
            edit_state.TextW.resize(buf_size + 1); // wchar count <= UTF-8
                                                   // count. we use +1 to make
            ...
            edit_state.CurLenW =
                ImTextStrFromUtf8(edit_state.TextW.Data, buf_size, buf, NULL,
                                  &buf_end);
            edit_state.CurLenA =
                (int)(buf_end - buf); // We can't get the result from
                                      // ImStrncpy() above because it is not
                                      // UTF-8 aware. Here we'll cut off
                                      // malformed UTF-8.
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_paste =
            ...
            is_editable;
        const bool is_undo =
            ((is_shortcut_key && IsKeyPressedMap(ImGuiKey_Z)) && is_editable &&
             is_undoable);
        const bool is_redo =
                          ...
                          ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    IM_ASSERT(callback_data.Buf ==
                              edit_state.TempBuffer.Data); // Invalid to modify
                                                           // those fields
    ...
    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y,
                           ...
                           frame_bb.Min.y + size.y); // Not using frame_bb.Max
                                                     // because we have adjusted
                                                     // size
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
...

        {
            ...
            // measuring distance
            cursor_offset.x =
                InputTextCalcTextSizeW(ImStrbolW(searches_input_ptr[0],
                                                 text_begin),
        ...
        if (edit_state.CursorFollow)
        {
                ...
                if (cursor_offset.x < edit_state.ScrollX)
                    edit_state.ScrollX =
                        (float)(int)ImMax(0.0f,
                                          cursor_offset.x - scroll_increment_x);
            ...
            if (is_multiline)
            {
                ...
                draw_window->DC.CursorPos.y +=
                    (draw_window->Scroll.y - scroll_y); // To avoid a frame of
                                                        // lag
        ...
        if (edit_state.StbState.select_start != edit_state.StbState.select_end)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                {
                    ImVec2 rect_size =
                        InputTextCalcTextSizeW(p, text_selected_end, &p, NULL,
                                               true);
                    if (rect_size.x <= 0.0f)
                        rect_size.x =
    ...
    if (is_multiline)
    {
        Dummy(text_size + ImVec2(0.0f, g.FontSize)); // Always add room to
                                                     // scroll an extra line
        EndChildFrame();
        EndGroup();
    }
                       ...
                       ImGuiColorEditFlags flags)
{
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const char* fmt_table_float[3][4] = {
            ...
            {"R:%0.3f", "G:%0.3f", "B:%0.3f", "A:%0.3f"}, // Long display for
                                                          // RGBA
            {"H:%0.3f", "S:%0.3f", "V:%0.3f", "A:%0.3f"}  // Long display for
                                                          // HSVA
        };
        const int fmt_idx =
    ...
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF)
    {
        ...
        for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++)
        {
                 ...
                 x < p_max.x; x += grid_step * 2.0f)
            {
                ...
                window->DrawList->AddRectFilled(ImVec2(x1, y1), ImVec2(x2, y2),
                                                col_bg2,
                                                rounding_corners_flags_cell
                                                    ? rounding
                                                    : 0.0f,
                         ...
                         ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        RenderFrameBorder(picker_pos,
                          ...
                          0.0f);
        sv_cursor_pos.x =
            ImClamp((float)(int)(picker_pos.x + ImSaturate(S) * sv_picker_size +
                                 0.5f),
                    picker_pos.x + 2,
                        ...
                        ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if (g.Style.FrameBorderSize > 0.0f)
        ...
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg),
                                  rounding); // Color button are often in need
                                             // of some sort of border
    ...
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable("##selectable", false, 0,
                           picker_size)) // By default, Selectable() is closing
                                         // popup
                g.ColorEditOptions =
                             ...
                             const char* label, const char* label_end)
{
    ...
    const float text_offset_x =
        (g.FontSize +
         (display_frame ? padding.x * 3 : padding.x * 2)); // Collapser arrow
                                                           // width + Spacing
    ...
    if (!is_leaf)
    {
        ...
        if (pressed)
        {
            ...
            if (g.DragDropActive && is_open) // When using Drag and Drop "hold
                                             // to open" we keep the node
                                             // highlighted after opening, but
...
void ImGui::TreePop()
{
    ...
    IM_ASSERT(window->IDStack.Size > 1); // There should always be 1 element in
                                         // the IDStack (pushed during window
                                         // creation). If this triggers you
                                         // called TreePop/PopID too much.
    PopID();
}
    ...
    if (values_count > 0)
    {
        ...
        ImVec2 tp0 =
            ImVec2(t0, 1.0f - ImSaturate((v0 - scale_min) *
                                         inv_scale)); // Point in the normalized
                                                      // space of our target
                                                      // rectangle
...
float ImGuiMenuColumns::DeclColumns(float w0, float w1,
                                    float w2) // not using va_arg because they
...
void ImGui::EndMenuBar()
{
    ...
    window->DC.MenuBarOffset.x =
        window->DC.CursorPos.x -
        window->MenuBarRect().Min.x; // Save horizontal position so next append
                                     // can reuse it. This is kinda equivalent
                                     // to a per-layer CursorPos.
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    {
        // Menu bar
        if (menu_is_open && pressed && menuset_is_open) // Click an open menu
                                                        // again to close it
        {
            want_close = true;
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label,
                                window->DC.ItemFlags |
                                    ...
                                    (menu_is_open ? ImGuiItemStatusFlags_Opened
                                                  : 0));
    ...
    if (menu_is_open)
    {
        ...
        menu_is_open =
            BeginPopupEx(id, flags); // menu_is_open can be 'false' when the
                     ...
                     bool enabled)
{
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(window->DC.LastItemId, label,
                                window->DC.ItemFlags |
                                    ImGuiItemStatusFlags_Checkable |
                                    (selected ? ImGuiItemStatusFlags_Checked
                                              : 0));
    return pressed;
}
...
void ImGui::SetTabItemClosed(const char* label)
{
    ...
    {
        ImGuiTabBar* tab_bar = g.CurrentTabBar.back();
        IM_ASSERT(tab_bar->WantLayout); // Needs to be called AFTER
                                        // BeginTabBar() and BEFORE the first
                                       ...
                                       ImGuiID close_button_id)
{
    ...
    if (flags & ImGuiTabItemFlags_UnsavedDocument)
    {
        ...
        ImVec2 unsaved_marker_pos(ImMin(bb.Min.x + style.FramePadding.x +
                                            label_size.x + 2,
                                        text_pixel_clip_bb.Max.x),
                                  bb.Min.y + style.FramePadding.y +
    ...
    if (label_size.x > text_ellipsis_clip_bb.GetWidth())
    {
        ...
        if (label_end == label &&
            label_end < label_display_end) // Always display at least 1
                                           // character if there's no room for
                                           // character + ellipsis
        {
            label_end =
