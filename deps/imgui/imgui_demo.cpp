...
void ImGui::ShowDemoWindow(bool* p_open)
{
    ...
    if (show_app_documents)
        ShowExampleAppDocuments(&show_app_documents); // Process the Document
                                                      // app next, as it may
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        {
            // List box
            const char* listbox_items[] = {"Apple",     "Banana",
                                           "Cherry",    "Kiwi",
                                           "Mango",     "Orange",
        ...
        if (ImGui::BeginTabItem("Rendering"))
        {
            ...
            ImGui::DragFloat("Global Alpha", &style.Alpha, 0.005f, 0.20f, 1.0f,
                             "%.2f"); // Not exposing zero here so user doesn't
                                      ...
                                      // have a toggle to switch between zero
                                      // and non-zero.
            ImGui::PopItemWidth();
...
struct ExampleAppConsole
{
    ...
    ExampleAppConsole()
    {
        ...
        Commands.push_back("CLASSIFY"); // "classify" is only here to provide an
    ...
    static int TextEditCallbackStub(
        ImGuiInputTextCallbackData* data) // In C++11 you are better off using
...
struct ExampleAppLog
{
    ...
    ImVector<int> LineOffsets; // Index to lines offset. We maintain this with
                               // AddLog() calls, allowing us to have a random
                               // access on lines
    bool ScrollToBottom;
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    if (type == 0)
        ImGui::SetNextWindowSizeConstraints(ImVec2(-1, 0),
                                            ImVec2(-1, FLT_MAX)); // Vertical
                                                                  // only
    if (type == 1)
        ImGui::SetNextWindowSizeConstraints(ImVec2(0, -1),
                                            ImVec2(FLT_MAX, -1)); // Horizontal
                                                                  // only
    if (type == 2)
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::Separator();
    {
        ...
        draw_list->PushClipRect(canvas_pos,
                                ImVec2(canvas_pos.x + canvas_size.x,
                                       canvas_pos.y + canvas_size.y),
                                true); // clip lines within the canvas (if we
                                       // resize it, etc.)
