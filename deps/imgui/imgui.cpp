...
ImGuiStyle::ImGuiStyle()
{
    ...
    FramePadding = ImVec2(4, 3); // Padding within a framed rectangle (used by
                                 // most widgets)
    ...
    DisplayWindowPadding =
        ImVec2(19, 19); // Window position are clamped to be visible within the
                        // display area by at least this amount. Only applies to
                        // regular windows.
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    window->DC.PrevLineSize.y =
        (line_height -
         GImGui->Style.ItemSpacing.y); // If we end up needing more accurate
                                       // data (to e.g. use SameLine) we may as
                                       // well make the clipper have a fourth
                                       // step to let user process and display
                                       // the last item in their list.
    if (window->DC.ColumnsSet)
        window->DC.ColumnsSet->LineMinY =
            window->DC.CursorPos.y; // Setting this so that cell Y position are
                                    // set properly
}
...
void ImGuiListClipper::Begin(int count, float items_height)
{
    ...
    if (ItemsHeight > 0.0f)
    {
        ImGui::CalcListClipping(ItemsCount, ItemsHeight, &DisplayStart,
                                &DisplayEnd); // calculate how many to
                                              // clip/display
        if (DisplayStart > 0)
...
void ImGui::RenderArrow(ImVec2 p_min, ImGuiDir dir, float scale)
{
    ...
    g.CurrentWindow->DrawList->AddTriangleFilled(center + a, center + b,
                                                 center + c,
                               ...
                               ImGuiNavHighlightFlags flags)
{
    ...
    if (flags & ImGuiNavHighlightFlags_TypeDefault)
    {
        ...
        window->DrawList->AddRect(display_rect.Min + ImVec2(THICKNESS * 0.5f,
                                                            THICKNESS * 0.5f),
                                  display_rect.Max - ImVec2(THICKNESS * 0.5f,
                                                            THICKNESS * 0.5f),
                                ...
                                ImGuiWindow* window)
{
    ...
    {
        ImGuiWindow* child = window->DC.ChildWindows[i];
        if (IsWindowActiveAndVisible(child)) // clipped children may have been
                                             // marked not active
...
void ImGui::EndFrame()
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(g.Initialized);
    if (g.FrameCountEnded == g.FrameCount) // Don't process EndFrame() multiple
                                           // times.
        return;
    ...
    IM_ASSERT(g.Windows.Size == g.WindowsSortBuffer.Size); // we done something
                                                           // wrong
    ...
    for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_front_most); n++)
        if (windows_to_render_front_most[n] &&
            IsWindowActiveAndVisible(
                windows_to_render_front_most[n])) // NavWindowingTarget is
                                                  // always temporarily
                                                  // displayed as the front-most
                                                  // window
                                ...
                                ImGuiWindowFlags flags)
{
    ...
    flags |= (parent_window->Flags & ImGuiWindowFlags_NoMove); // Inherit the
                                                               // NoMove flag
                                      ...
                                      ImU32 resize_grip_col[4])
{
    ...
    const float grip_draw_size =
        (float)(int)ImMax(g.FontSize * 1.35f,
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    if (extra_flags & ImGuiWindowFlags_ChildMenu)
        ...
        ImFormatString(name, IM_ARRAYSIZE(name), "##Popup_%08x",
                       id); // Not recycling, so we can close/open during the
                            // same frame
...
{
    ImGuiContext& g = *GImGui;
    if (g.OpenPopupStack.Size <= g.BeginPopupStack.Size) // Early out for
                                                         // performance
    {
        g.NextWindowData.Clear(); // We behave like Begin() and need to consume
                                  // those values
...
void ImGui::EndPopup()
{
    ImGuiContext& g = *GImGui;
    (void)g;
    IM_ASSERT(g.CurrentWindow->Flags & ImGuiWindowFlags_Popup); // Mismatched
                                                                // BeginPopup()/EndPopup()
                                                                // calls
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_ChildMenu)
    {
        ...
        float horizontal_overlap =
            g.Style.ItemSpacing.x; // We want some overlap to convey the
                                   // relative depth of each menu (currently the
                                   // amount of overlap is hard-coded to
                                   // style.ItemSpacing.x).
        ImRect r_avoid;
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    if (window->ParentWindow == g.NavWindow)
    {
        ...
        if (!window->ClipRect.Contains(cand))
            return false;
        cand.ClipWithFull(window->ClipRect); // This allows the scored item to
                                             // not overlap other candidates in
                                             // the parent window
    }
            ...
            else if (dist_center == result->DistCenter)
            {
                ...
                if (((g.NavMoveDir == ImGuiDir_Up ||
                      g.NavMoveDir == ImGuiDir_Down)
                         ? dby
                         : dbx) < 0.0f) // moving bj to the right/down decreases
                                        // distance
                    new_best = true;
            }
        }
    ...
    else
    {
        ...
        return ImFloor(ImClamp(pos, visible_rect.Min,
                               visible_rect.Max)); // ImFloor() is important
                                                   ...
                                                   // back-end might be lossy
                                                   // and result in undesirable
                                                   // non-zero delta.
    }
}
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode)
{
    ...
    if (mode == ImGuiInputReadMode_Repeat)
        return (
            float)CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime,
...
#undef NAV_MAP_KEY
    ...
    {
        ImGuiWindow* window = g.NavWindow;
        ImRect window_rect_rel(window->InnerMainRect.Min - window->Pos -
                                   ImVec2(1, 1),
                               window->InnerMainRect.Max - window->Pos +
                                   ImVec2(1, 1));
                ...
                ImVec2(-ImMin(window_rect_rel.GetWidth(), pad),
                       -ImMin(window_rect_rel.GetHeight(),
                              pad))); // Terrible approximation for the intent
                                      // of starting navigation from first fully
                                      // visible item
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir)
{
    ...
    ImGuiWindow* window_target =
        FindWindowNavFocusable(i_current + focus_change_dir, -INT_MAX,
                               focus_change_dir);
    if (!window_target)
        window_target =
        ...
        !(g.NavWindowingTarget->Flags & ImGuiWindowFlags_NoMove))
    {
        ...
        if (move_delta.x != 0.0f || move_delta.y != 0.0f)
        {
            ...
            const float move_speed = ImFloor(
                ...
                ImMin(g.IO.DisplayFramebufferScale.x,
                      g.IO.DisplayFramebufferScale.y)); // FIXME: Doesn't code
                                                        // variable framerate
                                                        // very well
                         ...
                         ImGuiColumnsFlags flags)
{
    ...
    IM_ASSERT(window->DC.ColumnsSet == NULL); // Nested columns are currently
                                              // not supported
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id != 0 && g.ActiveId != source_id) // Early out for most
                                                       // common case
            return false;
                               ...
                               size_t data_size, ImGuiCond cond)
{
    ...
    IM_ASSERT(payload.SourceId != 0); // Not called between
                                      // BeginDragDropSource() and
                                      // EndDragDropSource()
                                                 ...
                                                 ImGuiDragDropFlags flags)
{
    ...
    IM_ASSERT(g.DragDropActive); // Not called between BeginDragDropTarget() and
                                 // EndDragDropTarget() ?
    IM_ASSERT(payload.DataFrameCount != -1); // Forgot to call
                                             // EndDragDropTarget() ?
                                           ...
                                           ImGuiTextBuffer* buf)
{
    ...
    for (int i = 0; i != g.SettingsWindows.Size; i++)
    {
        ...
        if (const char* p =
                strstr(name, "###")) // Skip to the "###" marker if any. We
                                     // don't skip past to match the behavior of
                                     // GetID()
            name = p;
