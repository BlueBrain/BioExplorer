...
//  2018-06-12: DirectX12: Moved the ID3D12GraphicsCommandList* parameter from
//  NewFrame() to RenderDrawData(). 2018-06-08: Misc: Extracted
//  imgui_impl_dx12.cpp/.h away from the old combined DX12+Win32 example.
//  2018-06-08: DirectX12: Use draw_data->DisplayPos and draw_data->DisplaySize
//  to setup projection matrix and clipping rectangle (to ease support for
//  future multi-viewport). 2018-02-22: Merged into master with all Win32 code
                                   ...
                                   ID3D12GraphicsCommandList* ctx)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            {
                const D3D12_RECT r = {(LONG)(pcmd->ClipRect.x - pos.x),
                ...
                ctx->DrawIndexedInstanced(pcmd->ElemCount, 1, idx_offset,
                                          vtx_offset, 0);
            }
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        hr =
            g_pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
...
bool ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        ID3DBlob* blob = NULL;
        if (D3D12SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION_1,
                                        &blob, NULL) != S_OK)
            return false;
    ...
    // Create the vertex shader
    {
        ...
        D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main",
                   "vs_5_0", 0, 0, &g_pVertexShaderBlob, NULL);
        if (g_pVertexShaderBlob == NULL) // NB: Pass ID3D10Blob* pErrorBlob to
                                         // D3DCompile() to get error showing in
                                         // (const
    ...
    // Create the pixel shader
    {
        ...
        D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main",
                   "ps_5_0", 0, 0, &g_pPixelShaderBlob, NULL);
        if (g_pPixelShaderBlob == NULL) // NB: Pass ID3D10Blob* pErrorBlob to
                                        // D3DCompile() to get error showing in
                                        // (const
                                        // char*)pErrorBlob->GetBufferPointer().
                                        // Make sure to Release() the blob!
