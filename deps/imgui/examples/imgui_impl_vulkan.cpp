...
//  in the About Window. 2018-08-25: Vulkan: Fixed mishandled
                                     ...
                                     VkCommandBuffer command_buffer)
{
    ...
    if (!fd->VertexBuffer || fd->VertexBufferSize < vertex_size)
        CreateOrResizeBuffer(fd->VertexBuffer, fd->VertexBufferMemory,
                             fd->VertexBufferSize, vertex_size,
                             VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
    if (!fd->IndexBuffer || fd->IndexBufferSize < index_size)
        CreateOrResizeBuffer(fd->IndexBuffer, fd->IndexBufferMemory,
    ...
    // Upload Vertex and index Data:
    {
        ...
        err = vkMapMemory(g_Device, fd->VertexBufferMemory, 0, vertex_size, 0,
                          (void**)(&vtx_dst));
        check_vk_result(err);
        err = vkMapMemory(g_Device, fd->IndexBufferMemory, 0, index_size, 0,
                          (void**)(&idx_dst));
        check_vk_result(err);
    ...
    // Bind Vertex And Index Buffer:
    {
        ...
        vkCmdBindVertexBuffers(command_buffer, 0, 1, vertex_buffers,
                               vertex_offset);
        vkCmdBindIndexBuffer(command_buffer, fd->IndexBuffer, 0,
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                VkRect2D scissor;
                scissor.offset.x =
                    (int32_t)(pcmd->ClipRect.x - display_pos.x) > 0
                        ? (int32_t)(pcmd->ClipRect.x - display_pos.x)
                        : 0;
                scissor.offset.y =
                    (int32_t)(pcmd->ClipRect.y - display_pos.y) > 0
                ...
                // Draw
                vkCmdDrawIndexed(command_buffer, pcmd->ElemCount, 1, idx_offset,
                                 vtx_offset, 0);
            }
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    // Create the Upload Buffer:
    {
        ...
        check_vk_result(err);
        err = vkBindBufferMemory(g_Device, g_UploadBuffer, g_UploadBufferMemory,
                                 0);
        check_vk_result(err);
    }
...
void ImGui_ImplVulkan_InvalidateDeviceObjects()
{
    ...
    if (g_DescriptorSetLayout)
    {
        vkDestroyDescriptorSetLayout(g_Device, g_DescriptorSetLayout,
                                     g_Allocator);
...
void ImGui_ImplVulkanH_CreateWindowDataSwapChainAndFramebuffer(
    VkPhysicalDevice physical_device, VkDevice device,
    ImGui_ImplVulkanH_WindowData* wd, const VkAllocationCallbacks* allocator,
    int w, int h)
{
    uint32_t min_image_count =
        2; // FIXME: this should become a function parameter
    ...
    // Create Swapchain
    {
        ...
        err = vkGetSwapchainImagesKHR(device, wd->Swapchain,
                                      &wd->BackBufferCount, NULL);
        check_vk_result(err);
        err = vkGetSwapchainImagesKHR(device, wd->Swapchain,
                                      &wd->BackBufferCount, wd->BackBuffer);
        check_vk_result(err);
    }
    if (old_swapchain)
...
void ImGui_ImplVulkanH_DestroyWindowData(VkInstance instance, VkDevice device,
                                         ImGui_ImplVulkanH_WindowData* wd,
                                         const VkAllocationCallbacks* allocator)
{
    vkDeviceWaitIdle(device); // FIXME: We could wait on the Queue if we had the
                              // queue in wd-> (otherwise VulkanH functions
                              // can't use globals)
