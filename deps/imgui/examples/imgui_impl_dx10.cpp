    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                const D3D10_RECT r = {(LONG)(pcmd->ClipRect.x - pos.x),
...
bool ImGui_ImplDX10_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (g_pd3dDevice->CreateVertexShader(
                ...
                g_pVertexShaderBlob->GetBufferSize(), &g_pVertexShader) != S_OK)
            return false;

        // Create the input layout
        D3D10_INPUT_ELEMENT_DESC local_layout[] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
             (size_t)(&((ImDrawVert*)0)->pos), D3D10_INPUT_PER_VERTEX_DATA, 0},
            {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
             (size_t)(&((ImDrawVert*)0)->uv), D3D10_INPUT_PER_VERTEX_DATA, 0},
            {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
             (size_t)(&((ImDrawVert*)0)->col), D3D10_INPUT_PER_VERTEX_DATA, 0},
        };
    ...
    // Create the pixel shader
    {
        ...
        if (g_pd3dDevice->CreatePixelShader(
                ...
                g_pPixelShaderBlob->GetBufferSize(), &g_pPixelShader) != S_OK)
            return false;
    }
