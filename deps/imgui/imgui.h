...
#define IM_FMTARGS(FMT) \
    __attribute__((format(printf, FMT, FMT + 1))) // Apply printf-style warnings
                                                  // to user functions.
...
typedef unsigned int ImU32; // 32-bit unsigned integer (often used to store
                            // packed colors)
#if defined(_MSC_VER) && !defined(__clang__)
typedef signed __int64 ImS64; // 64-bit signed integer (pre and post C++11 with
                              // Visual Studio)
...
IMGUI_API void DestroyContext(ImGuiContext* ctx = NULL); // NULL = destroy
                                                         // current context
...
IMGUI_API ImDrawData* GetDrawData(); // valid after Render() and until the next
                                     ...
                                     // be passed to your io.RenderDrawListsFn()
                                     // function.)

// Demo, Debug, Information
IMGUI_API void ShowDemoWindow(
    bool* p_open = NULL); // create demo/test window (previously called
                          // ShowTestWindow). demonstrate most ImGui features.
                          // call this to learn about the library! try to make
...
IMGUI_API void ShowMetricsWindow(
    bool* p_open = NULL); // create metrics window. display Dear ImGui
                          // internals: draw commands (with individual draw
                          // calls and vertices), window list, basic internal
                          // state, etc.
IMGUI_API void ShowStyleEditor(
    ImGuiStyle* ref = NULL); // add style editor block (not a window). you can
                             // pass in a reference ImGuiStyle structure to
                             // compare to, revert to and save to (else it uses
                             // the default style)
...
// Styles
IMGUI_API void StyleColorsDark(ImGuiStyle* dst = NULL);    // new, recommended
                                                           // style (default)
IMGUI_API void StyleColorsClassic(ImGuiStyle* dst = NULL); // classic imgui
                                                           // style
IMGUI_API void StyleColorsLight(ImGuiStyle* dst = NULL);   // best used with
                                                           // borders and a
                                                         // custom, thicker font
...
IMGUI_API bool IsWindowFocused(
    ImGuiFocusedFlags flags = 0); // is current window focused? or its
                                  // root/child, depending on flags. see flags
                                  // for options.
IMGUI_API bool IsWindowHovered(
    ImGuiHoveredFlags flags = 0); // is current window hovered (and typically:
                                  ...
                                  // for options. NB: If you are trying to check
                                  // whether your mouse should be dispatched to
                                  // imgui or to your app, you should use the
                                  // 'io.WantCaptureMouse' boolean for that!
                                  // Please read the FAQ!
IMGUI_API ImDrawList* GetWindowDrawList(); // get draw list associated to the
                                           // window, to append your own drawing
                                           // primitives
...
IMGUI_API void SetNextWindowPos(
    ...
    const ImVec2& pivot = ImVec2(0, 0)); // set next window position. call
                                         // before Begin(). use
                                         // pivot=(0.5f,0.5f) to center on given
                                         // point, etc.
...
IMGUI_API void PushFont(ImFont* font); // use NULL as a shortcut to push default
                                       // font
...
IMGUI_API ImU32 GetColorU32(const ImVec4& col); // retrieve given color with
                                                // style alpha applied
IMGUI_API ImU32 GetColorU32(ImU32 col); // retrieve given color with style alpha
                                        // applied
...
IMGUI_API ImVec2 GetCursorStartPos();  // initial cursor position in window
                                       // coordinates
...
//   normally displayed.
IMGUI_API void PushID(const char* str_id); // push string identifier into the ID
                                           // stack. IDs == hash of the entire
                                           // stack!
...
IMGUI_API bool DragFloat(const char* label, float* v, float v_speed = 1.0f,
                         ...
                         float power = 1.0f); // If v_min >= v_max we have no
                                              // bound
...
IMGUI_API bool DragInt(const char* label, int* v, float v_speed = 1.0f,
                       ...
                       const char* format = "%d"); // If v_min >= v_max we have
                                                   // no bound
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
    IM_FMTARGS(2); // helper variation to completely decorelate the id from the
...
IMGUI_API void TreePush(const char* str_id); // ~ Indent()+PushId(). Already
                                             ...
                                             // returning true, but you can call
                                             // TreePush/TreePop yourself if
                                             // desired.
...
IMGUI_API bool CollapsingHeader(
    const char* label,
    ImGuiTreeNodeFlags flags = 0); // if returning 'true' the header is open.
                                   // doesn't indent nor push on ID stack. user
                                   // doesn't have to call TreePop().
IMGUI_API bool CollapsingHeader(
    const char* label, bool* p_open,
    ImGuiTreeNodeFlags flags = 0); // when 'p_open' isn't NULL, display an
                                   // additional small close button on upper
                                   // right of the header
...
IMGUI_API void OpenPopup(
    const char* str_id); // call to mark popup as open (don't call every
                         // frame!). popups are closed when user click outside,
                         // or if CloseCurrentPopup() is called within a
                         // BeginPopup()/EndPopup() block. By default,
                         // Selectable()/MenuItem() are calling
                         // CloseCurrentPopup(). Popup identifiers are relative
                         // to the current ID-stack (so OpenPopup and BeginPopup
...
IMGUI_API bool BeginPopupContextItem(
    ...
    int mouse_button = 1); // helper to open and begin popup when clicked on
                           ...
                           // need to pass in an explicit ID here. read comments
                           // in .cpp!
IMGUI_API bool BeginPopupContextWindow(
    const char* str_id = NULL, int mouse_button = 1,
    bool also_over_items = true); // helper to open and begin popup when clicked
                                  // on current window.
...
IMGUI_API bool BeginPopupModal(
    ...
    ImGuiWindowFlags flags = 0); // modal dialog (regular window with title bar,
                                 // block interactions behind the modal window,
                                 // can't close the modal window by clicking
                                 // outside)
...
IMGUI_API float GetColumnWidth(int column_index = -1); // get column width (in
                                                       // pixels). pass -1 to
                                                       // use current column
...
IMGUI_API float GetColumnOffset(
    int column_index = -1); // get position of column line (in pixels, from the
                            // left side of the contents region). pass -1 to use
                            // current column, otherwise 0..GetColumnsCount()
...
IMGUI_API bool BeginTabBar(const char* str_id,
                           ImGuiTabBarFlags flags = 0); // create and append
                                                        // into a TabBar
...
IMGUI_API bool BeginTabItem(const char* label, bool* p_open = NULL,
                            ImGuiTabItemFlags flags = 0); // create a Tab.
                                                          // Returns true if the
                                                          // Tab is selected.
...
IMGUI_API void LogToClipboard(int max_depth = -1);     // start logging to OS
                                                       // clipboard
...
IMGUI_API bool SetDragDropPayload(
    ...
    ImGuiCond cond = 0); // type is a user defined string of maximum 32
                         // characters. Strings starting with '_' are reserved
                         // for dear imgui internal types. Data is copied and
                         // held by imgui.
...
IMGUI_API bool IsItemHovered(
    ImGuiHoveredFlags flags = 0); // is the last item hovered? (and usable, aka
                                  // not blocked by a popup, etc.). See
...
IMGUI_API bool IsItemFocused(); // is the last item focused for keyboard/gamepad
                                // navigation?
IMGUI_API bool IsItemClicked(int mouse_button = 0); // is the last item clicked?
...
IMGUI_API bool IsRectVisible(
    ...
    const ImVec2& rect_max); // test if rectangle (in screen space) is visible /
                             // not clipped. to perform coarse clipping on
                             // user's side.
...
IMGUI_API bool IsKeyReleased(int user_key_index); // was key released (went from
                                                  // Down to !Down)..
...
IMGUI_API bool IsMouseDown(int button); // is mouse button held (0=left,
                                        // 1=right, 2=middle)
IMGUI_API bool IsAnyMouseDown();        // is any mouse button held
IMGUI_API bool IsMouseClicked(int button,
                              bool repeat = false); // did mouse button clicked
...
IMGUI_API bool IsMouseReleased(int button); // did mouse button released (went
                                            // from Down to !Down)
...
IMGUI_API ImVec2 GetMousePosOnOpeningCurrentPopup(); // retrieve backup of mouse
                                                     ...
                                                     // opening popup we have
                                                     // BeginPopup() into
IMGUI_API ImVec2 GetMouseDragDelta(
    int button = 0,
    float lock_threshold = -1.0f); // return the delta from the initial clicking
                                   // position. This is locked and return 0.0f
                                   // until the mouse moves past a distance
                                   // threshold at least once. If lock_threshold
                                   // < -1.0f uses io.MouseDraggingThreshold
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NoMouse =
        1 << 4, // Instruct imgui to clear mouse position/buttons in NewFrame().
                // This allows ignoring the mouse information set by the
                // back-end.
...
struct ImGuiIO
{
    ...
    ImGuiBackendFlags BackendFlags; // = 0              // See
                                    // ImGuiBackendFlags_ enum. Set by back-end
                                    // (imgui_impl_xxx files or custom back-end)
                                    // to communicate features supported by the
                                    // back-end.
    ...
    bool ConfigWindowsResizeFromEdges; // = true           // Enable resizing of
                                       ...
                                       // feedback. (This used to be the a
                                       // per-window
...
typedef ImGuiInputTextCallback ImGuiTextEditCallback; // OBSOLETE in 1.63 (from
                                                      // Aug 2018): made the
                                                      // names consistent
...
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data; // Pointer to shared draw data (you can
                                       ...
                                       // get the one from current ImGui
                                       // context)
    ...
    IMGUI_API void AddRect(
        ...
        float thickness = 1.0f); // a: upper-left, b: lower-right,
                                 // rounding_corners_flags: 4-bits corresponding
                                 // to which corner to round
    IMGUI_API void AddRectFilled(
        const ImVec2& a, const ImVec2& b, ImU32 col, float rounding = 0.0f,
        int rounding_corners_flags = ImDrawCornerFlags_All); // a: upper-left,
                                                             // b: lower-right
    ...
    IMGUI_API void PathArcToFast(const ImVec2& centre, float radius,
                                 int a_min_of_12,
                                 int a_max_of_12); // Use precomputed angles for
                                                   // a 12 steps circle
    ...
    IMGUI_API ImDrawList* CloneOutput() const; // Create a clone of the
                                               // CmdBuffer/IdxBuffer/VtxBuffer.
    ...
    IMGUI_API void PrimRect(const ImVec2& a, const ImVec2& b,
                            ImU32 col); // Axis aligned rectangle (composed of
                                        // two triangles)
...
struct ImDrawData
{
    ...
    IMGUI_API void ScaleClipRects(
        const ImVec2& sc); // Helper to scale the ClipRect field of each
                           // ImDrawCmd. Use if your final output buffer is at a
                           // different scale than ImGui expects, or if there is
                           // a difference between your window resolution and
...
struct ImFontConfig
{
    ...
    const ImWchar* GlyphRanges; // NULL     // Pointer to a user-provided list
                                // of Unicode range (2 value per range, values
                                // are inclusive, zero-terminated list). THE
                                // ARRAY DATA NEEDS TO PERSIST AS LONG AS THE
                                // FONT IS ALIVE.
...
{
    ImVector<int> UsedChars; // Store 1-bit per Unicode code point (0=unused,
                             // 1=used)
    ...
    IMGUI_API void AddRanges(const ImWchar* ranges);     // Add ranges, e.g.
                                                     ...
                                                     // to force add all of
                                                     // ASCII/Latin+Ext
    IMGUI_API void BuildRanges(ImVector<ImWchar>* out_ranges); // Output new
                                                               // ranges
};
...
struct ImFontAtlas
{
    ...
    IMGUI_API int AddCustomRectRegular(unsigned int id, int width,
                                       int height); // Id needs to be >=
                                                    // 0x10000. Id >= 0x80000000
                                                    // are reserved for ImGui
                                                    // and ImDrawList
    IMGUI_API int AddCustomRectFontGlyph(
        ...
        const ImVec2& offset = ImVec2(0, 0)); // Id needs to be < 0x10000 to
                                              // register a rectangle to map
                                              // into a specific font.
...
struct ImFont
{
    ...
    ImVector<float> IndexAdvanceX; //              // Sparse. Glyphs->AdvanceX
