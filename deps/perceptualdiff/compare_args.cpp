...
namespace pdiff
{
    ...
    static const auto USAGE =
        ...
        "  --fov deg         Field of view in degrees [0.1, 89.9] (default: "
        "45.0)\n"
        "  --threshold p     Number of pixels p below which differences are "
        "ignored\n"
        "  --gamma g         Value to convert rgb into linear space (default: "
        "2.2)\n"
        "  --luminance l     White luminance (default: 100.0 cdm^-2)\n"
        "  --luminance-only  Only consider luminance; ignore chroma (color) "
        "in the\n"
        "                    comparison\n"
        "  --color-factor    How much of color to use [0.0, 1.0] (default: "
        "1.0)\n"
        "  --down-sample     How many powers of two to down sample the image\n"
        "                    (default: 0)\n"
        "  --scale           Scale images to match each other's dimensions\n"
        "  --sum-errors      Print a sum of the luminance and color "
        "differences\n"
    ...
    CompareArgs::CompareArgs(int argc, char **argv)
        : verbose_(false), sum_errors_(false), down_sample_(0)
    {
    ...
    void CompareArgs::parse_args(const int argc, char **argv)
    {
        ...
        if (scale and (image_a_->get_width() != image_b_->get_width() or
        ...
        if (output_file_name)
        {
            image_difference_ = std::make_shared<RGBAImage>(
                image_a_->get_width(), image_a_->get_height(),
                output_file_name);
        }
    }
    ...
    {
        std::cout << "Field of view is " << parameters_.field_of_view
                  << " degrees\n"
