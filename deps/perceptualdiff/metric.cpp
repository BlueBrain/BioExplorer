...
namespace pdiff
{
                           ...
                           float &l, float &a, float &b)
    {
        ...
        const float r[] = { x / global_white.x, y / global_white.y,
                            z / global_white.z };
        float f[3];
        for (auto i = 0u; i < 3; i++)
        {
    ...
    PerceptualDiffParameters::PerceptualDiffParameters()
        : luminance_only(false), field_of_view(45.0f), gamma(2.2f),
          luminance(100.0f), threshold_pixels(100), color_factor(1.0f)
    {
    }


    bool yee_compare(const RGBAImage &image_a, const RGBAImage &image_b,
...
#pragma omp parallel for shared(args, a_lum, b_lum, a_a, a_b, b_a, b_b)
        for (auto y = 0; y < static_cast<ptrdiff_t>(h); y++)
        {
            for (auto x = 0u; x < w; x++)
            {
                ...
                const auto a_color_r =
                    powf(image_a.get_red(i) / 255.f * a_alpha, gamma);
                const auto a_color_g =
                    powf(image_a.get_green(i) / 255.f * a_alpha, gamma);
                const auto a_color_b =
                    powf(image_a.get_blue(i) / 255.f * a_alpha, gamma);

                float a_x;
                float a_y;
                float a_z;
                adobe_rgb_to_xyz(a_color_r, a_color_g, a_color_b, a_x, a_y,
                                 a_z);
                float l;
                ...
                const auto b_color_r =
                    powf(image_b.get_red(i) / 255.f * b_alpha, gamma);
                const auto b_color_g =
                    powf(image_b.get_green(i) / 255.f * b_alpha, gamma);
                const auto b_color_b =
                    powf(image_b.get_blue(i) / 255.f * b_alpha, gamma);

                float b_x;
                float b_y;
                float b_z;
                adobe_rgb_to_xyz(b_color_r, b_color_g, b_color_b, b_x, b_y,
                                 b_z);
        ...
        const auto num_one_degree_pixels =
            to_degrees(2 * std::tan(args.field_of_view * to_radians(.5f)));
