...
namespace pdiff
{
    ...
    static std::shared_ptr<FIBITMAP> to_free_image(const RGBAImage &image)
    {
        ...
        {
            auto scanline =
                reinterpret_cast<unsigned int *>(FreeImage_GetScanLine(
                    bitmap.get(), image.get_height() - y - 1));
    ...
    to_rgba_image(FIBITMAP *image, const std::string &filename = "")
    {
        ...
        // Copy the image over to our internal format, FreeImage has the
        // scanlines bottom to top though.
