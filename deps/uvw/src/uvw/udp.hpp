...
{
    explicit UDPDataEvent(Addr addr, std::unique_ptr<const char[]> ptr,
                          ssize_t l, bool trunc) noexcept
        : dt{std::move(ptr)}
        , len{l}
        , sndr(addr)
        , part{trunc}
...
public:
    ...
    void send(uv_udp_t *handle, const uv_buf_t bufs[], unsigned int nbufs,
              const struct sockaddr *addr)
    {
        invoke(&uv_udp_send, get<uv_udp_send_t>(), handle, bufs, nbufs, addr,
               &defaultCallback<uv_udp_send_t, SendEvent>);
    }
};
...
{
    template <typename I>
    static void recvCallback(uv_udp_t *handle, ssize_t nread,
                             const uv_buf_t *buf, const sockaddr *addr,
                             unsigned flags)
    {
        typename details::IpTraits<I>::Type *aptr =
            reinterpret_cast<const typename details::IpTraits<I>::Type *>(addr);
        ...
        if (nread > 0)
        {
            ...
            udp.publish(UDPDataEvent{details::address<I>(aptr), std::move(data),
                                     nread, flags & UV_UDP_PARTIAL});
        }
...
public:
    ...
    {
        return std::shared_ptr<UDPHandle>{
            new UDPHandle{std::forward<Args>(args)...}};
    }
    ...
    bool init()
    {
        return (tag == FLAGS) ? initialize<uv_udp_t>(&uv_udp_init_ex, flags)
              ...
              Flags<Bind> flags = Flags<Bind>{})
    {
        ...
        invoke(&uv_udp_bind, get<uv_udp_t>(),
     ...
     */
    template <typename I = IPv4>
    bool multicastMembership(std::string multicast, std::string interface,
                             Membership membership)
    {
        return (0 ==
                uv_udp_set_membership(get<uv_udp_t>(), multicast.data(),
                                      interface.data(),
    ...
    {
        return (0 == uv_udp_set_multicast_ttl(get<uv_udp_t>(),
                                              val > 255 ? 255 : val));
    }
    ...
    {
        return (0 == uv_udp_set_multicast_interface(get<uv_udp_t>(),
                                                    interface.data()));
    }
     ...
     */
    template <typename I = IPv4>
    void send(std::string ip, unsigned int port, std::unique_ptr<char[]> data,
              ssize_t len)
    {
        typename details::IpTraits<I>::Type addr;
        details::IpTraits<I>::addrFunc(ip.data(), port, &addr);
     ...
     */
    template <typename I = IPv4>
    int trySend(std::string ip, unsigned int port, std::unique_ptr<char[]> data,
                ssize_t len)
    {
        typename details::IpTraits<I>::Type addr;
        details::IpTraits<I>::addrFunc(ip.data(), port, &addr);
        ...
        uv_buf_t bufs[] = {uv_buf_init(data.get(), len)};
        auto bw = uv_udp_try_send(get<uv_udp_t>(), bufs, 1,
                                  reinterpret_cast<const sockaddr *>(&addr));
    ...
    void recv()
    {
        invoke(&uv_udp_recv_start, get<uv_udp_t>(), &allocCallback,
               &recvCallback<I>);
    }
