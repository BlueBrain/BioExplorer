...
public:
    ...
    {
        invoke(std::forward<F>(f), get<uv_connect_t>(),
               std::forward<Args>(args)...,
               &defaultCallback<uv_connect_t, ConnectEvent>);
    }
};
...
public:
    ...
    {
        invoke(&uv_shutdown, get<uv_shutdown_t>(), handle,
               &defaultCallback<uv_shutdown_t, ShutdownEvent>);
    }
};
...
public:
    ...
    {
        invoke(&uv_write, get<uv_write_t>(), handle, bufs, nbufs,
               &defaultCallback<uv_write_t, WriteEvent>);
    }

    void write(uv_stream_t *handle, const uv_buf_t bufs[], unsigned int nbufs,
               uv_stream_t *send)
    {
        invoke(&uv_write2, get<uv_write_t>(), handle, bufs, nbufs, send,
               &defaultCallback<uv_write_t, WriteEvent>);
    }
};
...
class StreamHandle : public Handle<T, U>
{
    ...
    static void readCallback(uv_stream_t *handle, ssize_t nread,
                             const uv_buf_t *buf)
    {
...
public:
    ...
    {
        auto listener = [ptr =
                             this->shared_from_this()](const auto &event,
                                                       details::ShutdownReq &) {
            ptr->publish(event);
        };
    ...
    {
        this->invoke(&uv_listen, this->template get<uv_stream_t>(), backlog,
                     &listenCallback);
    }
    ...
    void accept(S &ref)
    {
        this->invoke(&uv_accept, this->template get<uv_stream_t>(),
                     ref.template get<uv_stream_t>());
    }
    ...
    void read()
    {
        this->invoke(&uv_read_start, this->template get<uv_stream_t>(),
                     &this->allocCallback, &readCallback);
    }
    ...
    void write(std::unique_ptr<char[]> data, ssize_t len)
    {
        ...
        auto listener = [ptr = this->shared_from_this()](const auto &event,
                                                         details::WriteReq &) {
            ptr->publish(event);
        };
    ...
    {
        return (0 == uv_stream_set_blocking(this->template get<uv_stream_t>(),
                                            enable));
    }
};
