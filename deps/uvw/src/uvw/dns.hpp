...
{
    static void getAddrInfoCallback(uv_getaddrinfo_t *req, int status,
                                    addrinfo *res)
    {
    ...
    void getNodeAddrInfo(const char *node, const char *service,
                         addrinfo *hints = nullptr)
    {
        invoke(&uv_getaddrinfo, parent(), get<uv_getaddrinfo_t>(),
               &getAddrInfoCallback, node, service, hints);
    }

    auto getNodeAddrInfoSync(const char *node, const char *service,
                             addrinfo *hints = nullptr)
    {
        auto req = get<uv_getaddrinfo_t>();
        auto err = uv_getaddrinfo(parent(), req, nullptr, node, service, hints);
        auto ptr = std::unique_ptr<addrinfo, void (*)(addrinfo *)>{
            req->addrinfo, [](addrinfo *res) { uv_freeaddrinfo(res); }};
...
public:
    ...
    {
        return std::shared_ptr<GetAddrInfoReq>{
            new GetAddrInfoReq{std::move(loop)}};
    }
     ...
     * type constraints.
     */
    void getAddrInfo(std::string node, std::string service,
                     addrinfo *hints = nullptr)
    {
     ...
     * type constraints.
     */
    auto getAddrInfoSync(std::string node, std::string service,
                         addrinfo *hints = nullptr)
    {
...
{
    static void getNameInfoCallback(uv_getnameinfo_t *req, int status,
...
public:
    ...
    {
        return std::shared_ptr<GetNameInfoReq>{
            new GetNameInfoReq{std::move(loop)}};
    }
    ...
    void getNameInfo(std::string ip, unsigned int port, int flags = 0)
    {
        ...
        invoke(&uv_getnameinfo, parent(), get<uv_getnameinfo_t>(),
               &getNameInfoCallback, &addr, flags);
    }
    ...
    auto getNameInfoSync(std::string ip, unsigned int port, int flags = 0)
    {
        ...
        return std::make_pair(ErrorEvent{err},
                              NameInfoEvent{req->host, req->service});
    }
