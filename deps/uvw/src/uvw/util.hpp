...
Addr address(F &&f, const H *handle) noexcept
{
    ...
    int err =
        std::forward<F>(f)(handle, reinterpret_cast<sockaddr *>(&ssto), &len);

    if (0 == err)
    {
        typename IpTraits<I>::Type *aptr =
            reinterpret_cast<typename IpTraits<I>::Type *>(&ssto);
        addr = address<I>(aptr);
    }
...
struct Utilities
{
    ...
    static std::vector<InterfaceAddress> interfaceAddresses() noexcept
    {
        ...
        {
            std::for_each(
                ifaces, ifaces + count, [&interfaces](const auto &iface) {
     ...
     */
    static bool replaceAllocator(MallocFuncType mallocFunc,
                                 ReallocFuncType reallocFunc,
                                 CallocFuncType callocFunc,
                                 FreeFuncType freeFunc) noexcept
    {
        return (0 == uv_replace_allocator(mallocFunc, reallocFunc, callocFunc,
                                          freeFunc));
    }
     ...
     */
    static uint64_t totalMemory() noexcept { return uv_get_total_memory(); }
     ...
     */
    static uint64_t hrtime() noexcept { return uv_hrtime(); }

    /**
     * @brief Gets the executable path.
     * @return The executable path, an empty string in case of errors.
     */
    static std::string exepath() noexcept { return details::path(&uv_exepath); }

    /**
     * @brief Gets the current working directory.
     * @return The current working directory, an empty string in case of errors.
     */
    static std::string cwd() noexcept { return details::path(&uv_cwd); }
