...
protected:
    ...
    static void allocCallback(uv_handle_t *, std::size_t suggested,
                              uv_buf_t *buf)
    {
    ...
    bool initialize(F &&f, Args &&... args)
    {
        ...
        {
            auto err =
                std::forward<F>(f)(this->parent(), this->template get<H>(),
...
public:
    ...
    void close() noexcept override
    {
        if (!closing())
        {
            uv_close(this->template get<uv_handle_t>(),
                     &Handle<T, U>::closeCallback);
        }
    }
    ...
    int sendBufferSize() const
    {
        int value = 0;
        auto err =
            uv_send_buffer_size(this->template get<uv_handle_t>(), &value);
        return err ? 0 : value;
    }
    ...
    int recvBufferSize() const
    {
        int value = 0;
        auto err =
            uv_recv_buffer_size(this->template get<uv_handle_t>(), &value);
        return err ? 0 : value;
    }
