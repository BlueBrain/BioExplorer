...
{
    FsEvent(const char *p, std::unique_ptr<const char[]> ptr,
            ssize_t l) noexcept
        : rPath{p}
        , dt{std::move(ptr)}
        , len{l}
...
protected:
    ...
    {
        auto ptr =
            Request<T, uv_fs_t>::reserve(reinterpret_cast<uv_req_t *>(req));
        if (req->result < 0)
        {
    ...
    static void fsResultCallback(uv_fs_t *req)
    {
        ...
        {
            ptr->publish(FsEvent<e>{req->path, req->result});
        }
    }
...
class FileReq final : public FsRequest<FileReq>
{
    ...
    static void fsReadCallback(uv_fs_t *req)
    {
        ...
        {
            ptr->publish(FsEvent<Type::READ>{req->path, std::move(ptr->data),
                                             req->result});
        }
    }
...
public:
    ...
    ~FileReq() noexcept { uv_fs_req_cleanup(get<uv_fs_t>()); }
    ...
    void close()
    {
        cleanupAndInvoke(&uv_fs_close, parent(), get<uv_fs_t>(), file,
                         &fsCloseCallback);
    }
    ...
    auto closeSync()
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::CLOSE>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_open, parent(), get<uv_fs_t>(), path.data(),
                         flags, mode, &fsOpenCallback);
    }
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_open, parent(), req, path.data(), flags,
                             mode);
        if (req->result >= 0)
        {
    ...
    void read(int64_t offset, unsigned int len)
    {
        ...
        cleanupAndInvoke(&uv_fs_read, parent(), get<uv_fs_t>(), file, bufs, 1,
                         offset, &fsReadCallback);
    }
    ...
    auto readSync(int64_t offset, unsigned int len)
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::READ>{req->path, std::move(data),
                                                  req->result});
    }
    ...
    void write(std::unique_ptr<char[]> data, ssize_t len, int64_t offset)
    {
        ...
        cleanupAndInvoke(&uv_fs_write, parent(), get<uv_fs_t>(), file, bufs, 1,
                         offset, &fsResultCallback<Type::WRITE>);
    }
    ...
    auto writeSync(std::unique_ptr<char[]> data, ssize_t len, int64_t offset)
    {
        uv_buf_t bufs[] = {uv_buf_init(data.get(), len)};
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_write, parent(), get<uv_fs_t>(), file, bufs,
                             1, offset);
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::WRITE>{req->path, req->result});
    }
    ...
    void fstat()
    {
        cleanupAndInvoke(&uv_fs_fstat, parent(), get<uv_fs_t>(), file,
                         &fsStatCallback<Type::FSTAT>);
    }
    ...
    auto fstatSync()
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::FSTAT>{req->path, req->statbuf});
    }
    ...
    void fsync()
    {
        cleanupAndInvoke(&uv_fs_fsync, parent(), get<uv_fs_t>(), file,
                         &fsGenericCallback<Type::FSYNC>);
    }
    ...
    void fdatasync()
    {
        cleanupAndInvoke(&uv_fs_fdatasync, parent(), get<uv_fs_t>(), file,
                         &fsGenericCallback<Type::FDATASYNC>);
    }
    ...
    auto fdatasyncSync()
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::FDATASYNC>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_ftruncate, parent(), get<uv_fs_t>(), file,
                         offset, &fsGenericCallback<Type::FTRUNCATE>);
    }
    ...
    auto ftruncateSync(int64_t offset)
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::FTRUNCATE>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_sendfile, parent(), get<uv_fs_t>(), out, file,
                         offset, length, &fsResultCallback<Type::SENDFILE>);
    }
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_sendfile, parent(), req, out, file, offset,
                             length);
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::SENDFILE>{req->path, req->result});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_fchmod, parent(), get<uv_fs_t>(), file, mode,
                         &fsGenericCallback<Type::FCHMOD>);
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_futime, parent(), get<uv_fs_t>(), file,
                         atime.count(), mtime.count(),
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_futime, parent(), req, file, atime.count(),
                             mtime.count());
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::FUTIME>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_fchown, parent(), get<uv_fs_t>(), file, uid,
                         gid, &fsGenericCallback<Type::FCHOWN>);
    }
...
class FsReq final : public FsRequest<FsReq>
{
    static void fsReadlinkCallback(uv_fs_t *req)
    {
        ...
        {
            ptr->publish(FsEvent<Type::READLINK>{req->path,
                                                 static_cast<char *>(req->ptr),
                                                 req->result});
        }
    }
...
public:
    ...
    ~FsReq() noexcept { uv_fs_req_cleanup(get<uv_fs_t>()); }
    ...
    {
        cleanupAndInvoke(&uv_fs_unlink, parent(), get<uv_fs_t>(), path.data(),
                         &fsGenericCallback<Type::UNLINK>);
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_mkdir, parent(), get<uv_fs_t>(), path.data(),
                         mode, &fsGenericCallback<Type::MKDIR>);
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_mkdtemp, parent(), get<uv_fs_t>(), tpl.data(),
                         &fsGenericCallback<Type::MKDTEMP>);
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_rmdir, parent(), get<uv_fs_t>(), path.data(),
                         &fsGenericCallback<Type::RMDIR>);
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_scandir, parent(), get<uv_fs_t>(), path.data(),
                         flags, &fsResultCallback<Type::SCANDIR>);
    }
    ...
    auto scandirSync(std::string path, int flags)
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::SCANDIR>{req->path, req->result});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_stat, parent(), get<uv_fs_t>(), path.data(),
                         &fsStatCallback<Type::STAT>);
    }
    ...
    auto statSync(std::string path)
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::STAT>{req->path, req->statbuf});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_lstat, parent(), get<uv_fs_t>(), path.data(),
                         &fsStatCallback<Type::LSTAT>);
    }
    ...
    auto lstatSync(std::string path)
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::LSTAT>{req->path, req->statbuf});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_rename, parent(), get<uv_fs_t>(), old.data(),
                         path.data(), &fsGenericCallback<Type::RENAME>);
    }
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_rename, parent(), req, old.data(),
                             path.data());
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::RENAME>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_access, parent(), get<uv_fs_t>(), path.data(),
                         mode, &fsGenericCallback<Type::ACCESS>);
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_chmod, parent(), get<uv_fs_t>(), path.data(),
                         mode, &fsGenericCallback<Type::CHMOD>);
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_utime, parent(), get<uv_fs_t>(), path.data(),
                         atime.count(), mtime.count(),
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_utime, parent(), req, path.data(),
                             atime.count(), mtime.count());
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::UTIME>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_link, parent(), get<uv_fs_t>(), old.data(),
                         path.data(), &fsGenericCallback<Type::LINK>);
    }
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_link, parent(), req, old.data(),
                             path.data());
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::LINK>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_symlink, parent(), get<uv_fs_t>(), old.data(),
                         path.data(), flags, &fsGenericCallback<Type::SYMLINK>);
    }
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_symlink, parent(), req, old.data(),
                             path.data(), flags);
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::SYMLINK>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_readlink, parent(), get<uv_fs_t>(), path.data(),
                         &fsReadlinkCallback);
    }
    ...
    auto readlinkSync(std::string path)
    {
        ...
        return std::make_pair(
            ...
            FsEvent<Type::READLINK>{req->path, static_cast<char *>(req->ptr),
                                    req->result});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_realpath, parent(), get<uv_fs_t>(), path.data(),
                         &fsGenericCallback<Type::REALPATH>);
    }
    ...
    auto realpathSync(std::string path)
    {
        ...
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::REALPATH>{req->path});
    }
    ...
    {
        cleanupAndInvoke(&uv_fs_chown, parent(), get<uv_fs_t>(), path.data(),
                         uid, gid, &fsGenericCallback<Type::CHOWN>);
    }
    ...
    {
        auto req = get<uv_fs_t>();
        cleanupAndInvokeSync(&uv_fs_chown, parent(), req, path.data(), uid,
                             gid);
        return std::make_pair(ErrorEvent{req->result},
                              FsEvent<Type::CHOWN>{req->path});
    }
};
