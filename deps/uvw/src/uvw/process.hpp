...
{
    static void exitCallback(uv_process_t *handle, int64_t exitStatus,
                             int termSignal)
    {
...
public:
    ...
    {
        return std::shared_ptr<ProcessHandle>{
            new ProcessHandle{std::move(loop)}};
    }
     ...
     */
    int pid() noexcept { return get<uv_process_t>()->pid; }
