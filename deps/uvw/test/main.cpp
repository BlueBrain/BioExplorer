...
void listen(uvw::Loop &loop)
{
    ...
    tcp->once<uvw::ListenEvent>([](const uvw::ListenEvent &event,
                                   uvw::TcpHandle &srv) mutable {
        ...
        client->on<uvw::EndEvent>([](const uvw::EndEvent &,
    ...
    tcp->once<uvw::CloseEvent>(
        [](const uvw::CloseEvent &, uvw::TcpHandle &) mutable {
            std::cout << "close" << std::endl;
        });
...
void conn(uvw::Loop &loop)
{
    ...
    tcp->once<uvw::WriteEvent>(
        [](const uvw::WriteEvent &, uvw::TcpHandle &tcp) mutable {
            std::cout << "write" << std::endl;
            tcp.close();
        });

    tcp->once<uvw::ConnectEvent>(
        [](const uvw::ConnectEvent &, uvw::TcpHandle &tcp) mutable {
