...
UV_UNUSED(static int uv_spinlock_trylock(uv_spinlock_t* spinlock))
{
    /* TODO(bnoordhuis) Maybe change to a ticket lock to guarantee fair
     * queueing. Not really critical until we have locks that are (frequently)
     * contended for by several threads.
     */
