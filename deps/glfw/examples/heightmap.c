...
static void init_map(void)
{
    ...
    for (i = 0; i < MAP_NUM_VERTICES - 1; ++i)
    {
        ...
        map_line_indices[k++] =
            (MAP_NUM_VERTICES - 1) * MAP_NUM_VERTICES + i + 1;
    }
    ...
    for (k = 0; k < 2 * MAP_NUM_LINES; k += 2)
    {
        ...
        printf("Line %d: %d -> %d (%f, %f, %f) -> (%f, %f, %f)\n", k / 2, beg,
               end, map_vertices[0][beg], map_vertices[1][beg],
               map_vertices[2][beg], map_vertices[0][end], map_vertices[1][end],
               map_vertices[2][end]);
    }
#endif
}
...
static void update_map(int num_iter)
{
    ...
    while (num_iter)
    {
        ...
        for (ii = 0u; ii < MAP_NUM_TOTAL_VERTICES; ++ii)
        {
            ...
            GLfloat pd =
                (2.0f * (float)sqrt((dx * dx) + (dz * dz))) / circle_size;
            if (fabs(pd) <= 1.0f)
            {
