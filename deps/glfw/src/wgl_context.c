...
static int choosePixelFormat(_GLFWwindow* window, const _GLFWfbconfig* desired)
{
    ...
    {
        nativeCount =
            getPixelFormatAttrib(window, 1, WGL_NUMBER_PIXEL_FORMATS_ARB);
    }
    else
    {
    ...
    for (i = 0; i < nativeCount; i++)
    {
        ...
        if (_glfw.wgl.ARB_pixel_format)
        {
            ...
            u->stencilBits =
                getPixelFormatAttrib(window, n, WGL_STENCIL_BITS_ARB);

            u->accumRedBits =
                getPixelFormatAttrib(window, n, WGL_ACCUM_RED_BITS_ARB);
            u->accumGreenBits =
            ...
            u->auxBuffers =
                getPixelFormatAttrib(window, n, WGL_AUX_BUFFERS_ARB);
            ...
            {
                if (getPixelFormatAttrib(window, n,
        ...
        else
        {
            ...
            if (!DescribePixelFormat(window->context.wgl.dc, n,
...
GLFWbool _glfwInitWGL(void)
{
    ...
    _glfw.wgl.CreateContext =
        (WGLCREATECONTEXT_T)GetProcAddress(_glfw.wgl.instance,
                                           "wglCreateContext");
    _glfw.wgl.DeleteContext =
        (WGLDELETECONTEXT_T)GetProcAddress(_glfw.wgl.instance,
                                           "wglDeleteContext");
    _glfw.wgl.GetProcAddress =
        (WGLGETPROCADDRESS_T)GetProcAddress(_glfw.wgl.instance,
                                            "wglGetProcAddress");
    _glfw.wgl.GetCurrentDC =
        (WGLGETCURRENTDC_T)GetProcAddress(_glfw.wgl.instance,
                                          "wglGetCurrentDC");
    _glfw.wgl.MakeCurrent =
        (WGLMAKECURRENT_T)GetProcAddress(_glfw.wgl.instance, "wglMakeCurrent");
    _glfw.wgl.ShareLists =
        (WGLSHARELISTS_T)GetProcAddress(_glfw.wgl.instance, "wglShareLists");
                               ...
                               const _GLFWfbconfig* fbconfig)
{
    ...
    if (_glfw.wgl.ARB_create_context)
    {
        ...
        window->context.wgl.handle =
            _glfw.wgl.CreateContextAttribsARB(window->context.wgl.dc, share,
                                              attribs);
        if (!window->context.wgl.handle)
        {
            ...
            else if (error == (0xc0070000 | ERROR_INVALID_PROFILE_ARB))
            {
                _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                "WGL: Driver does not support the requested "
                                "OpenGL profile");
            }
            else
            {
