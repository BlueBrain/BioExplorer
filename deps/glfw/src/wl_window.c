...
static void checkScaleChange(_GLFWwindow* window)
{
    ...
    if (scale != window->wl.scale)
    {
        ...
        wl_egl_window_resize(window->wl.native, scaledWidth, scaledHeight, 0,
                             0);
        _glfwInputFramebufferSize(window, scaledWidth, scaledHeight);
    }
}

static void handleEnter(void* data, struct wl_surface* surface,
...
static void handleLeave(void* data, struct wl_surface* surface,
                        struct wl_output* output)
{
    _GLFWwindow* window = data;
    _GLFWmonitor* monitor = wl_output_get_user_data(output);
...
static const struct wl_surface_listener surfaceListener = {handleEnter,
                                                           handleLeave};
                              ...
                              const _GLFWwndconfig* wndconfig)
{
    ...
    window->wl.native =
        wl_egl_window_create(window->wl.surface, wndconfig->width,
                             wndconfig->height);
    if (!window->wl.native)
        return GLFW_FALSE;
                              ...
                              int xhot, int yhot)
{
    ...
    cursor->wl.buffer =
        wl_shm_pool_create_buffer(pool, 0, image->width, image->height, stride,
                                 ...
                                 wl_fixed_t dxUnaccel, wl_fixed_t dyUnaccel)
{
    ...
    _glfwInputCursorPos(
        window, window->virtualCursorPosX + wl_fixed_to_double(dxUnaccel),
...
void _glfwPlatformSetCursor(_GLFWwindow* window, _GLFWcursor* cursor)
{
    ...
    if (window->cursorMode == GLFW_CURSOR_NORMAL)
    {
        ...
        if (image)
        {
            ...
            wl_pointer_set_cursor(_glfw.wl.pointer, _glfw.wl.pointerSerial,
                                  surface, image->hotspot_x, image->hotspot_y);
        ...
        {
            wl_pointer_set_cursor(_glfw.wl.pointer, _glfw.wl.pointerSerial,
                                  surface, cursor->wl.xhot, cursor->wl.yhot);
            wl_surface_attach(surface, cursor->wl.buffer, 0, 0);
            wl_surface_damage(surface, 0, 0, cursor->wl.width,
                              cursor->wl.height);
            wl_surface_commit(surface);
        }
    }
    ...
    {
        wl_pointer_set_cursor(_glfw.wl.pointer, _glfw.wl.pointerSerial, NULL, 0,
                              0);
    }
}
                                                      ...
                                                      uint32_t queuefamily)
{
    ...
    if (!vkGetPhysicalDeviceWaylandPresentationSupportKHR)
    {
        _glfwInputError(GLFW_API_UNAVAILABLE,
                        "Wayland: Vulkan instance missing "
