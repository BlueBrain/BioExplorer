...
GLFWbool _glfwInitJoysticksLinux(void)
{
    ...
    _glfw.linux_js.watch = inotify_add_watch(_glfw.linux_js.inotify, dirname,
                                             IN_CREATE | IN_ATTRIB);
    ...
    if (dir)
    {
        ...
        while ((entry = readdir(dir)))
        {
            ...
            if (regexec(&_glfw.linux_js.regex, entry->d_name, 1, &match, 0) !=
                0)
                continue;
    ...
    else
    {
        _glfwInputError(
            GLFW_PLATFORM_ERROR,
            "Linux: Failed to open joystick device directory %s: %s", dirname,
            strerror(errno));
        // Continue with no joysticks detected
    }

    qsort(_glfw.linux_js.js, count, sizeof(_GLFWjoystickLinux),
          compareJoysticks);
#endif // __linux__
