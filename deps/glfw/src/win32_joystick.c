...
static BOOL CALLBACK deviceCallback(const DIDEVICEINSTANCE* di, void* user)
{
    ...
    {
        if (memcmp(&_glfw.win32_js[joy].guid, &di->guidInstance,
                   sizeof(GUID)) == 0)
            return DIENUM_CONTINUE;
    }
    ...
    if (FAILED(IDirectInputDevice8_EnumObjects(device, deviceObjectCallback,
                                               &data,
...
static GLFWbool openXinputDevice(DWORD index)
{
    ...
    for (joy = GLFW_JOYSTICK_1; joy <= GLFW_JOYSTICK_LAST; joy++)
    {
        if (_glfw.win32_js[joy].present && _glfw.win32_js[joy].device == NULL &&
...
static GLFWbool pollJoystickState(_GLFWjoystickWin32* js, int mode)
{
    ...
    if (js->device)
    {
        ...
        result = IDirectInputDevice8_GetDeviceState(js->device, sizeof(state),
                                                    &state);
    ...
    else
    {
        ...
        DWORD result;
        XINPUT_STATE xis;
        const WORD buttons[14] = {XINPUT_GAMEPAD_A,
                                  XINPUT_GAMEPAD_B,
                                  XINPUT_GAMEPAD_X,
                                  XINPUT_GAMEPAD_Y,
...
void _glfwInitJoysticksWin32(void)
{
    ...
    {
        if (FAILED(_glfw_DirectInput8Create(
                GetModuleHandle(NULL), DIRECTINPUT_VERSION, &IID_IDirectInput8W,
