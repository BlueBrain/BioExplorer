...
static void createKeyTables(void)
{
    ...
    if (_glfw.x11.xkb.available)
    {
        ...
        for (scancode = desc->min_key_code; scancode <= desc->max_key_code;
             scancode++)
        {
...
static void detectEWMH(void)
{
    ...
    if (_glfwGetWindowPropertyX11(_glfw.x11.root, supportingWmCheck, XA_WINDOW,
    ...
    if (_glfwGetWindowPropertyX11(*windowFromRoot, supportingWmCheck, XA_WINDOW,
    ...
    atomCount = _glfwGetWindowPropertyX11(_glfw.x11.root, wmSupported, XA_ATOM,
    ...
    _glfw.x11.NET_WM_STATE_MAXIMIZED_VERT =
        getSupportedAtom(supportedAtoms, atomCount,
...
static GLFWbool initExtensions(void)
{
    ...
    _glfw.x11.xkb.available =
        XkbQueryExtension(_glfw.x11.display, &_glfw.x11.xkb.majorOpcode,
                          &_glfw.x11.xkb.eventBase, &_glfw.x11.xkb.errorBase,
                          &_glfw.x11.xkb.major, &_glfw.x11.xkb.minor);
    ...
    if (_glfw.x11.x11xcb.handle)
    {
        _glfw.x11.x11xcb.XGetXCBConnection =
            (XGETXCBCONNECTION_T)dlsym(_glfw.x11.x11xcb.handle,
                                       "XGetXCBConnection");
    }
...
{
    char buffer[8192];
    XGetErrorText(_glfw.x11.display, _glfw.x11.errorCode, buffer,
                  sizeof(buffer));
