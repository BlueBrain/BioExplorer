...
//
static void getFullWindowSize(DWORD style, DWORD exStyle, int clientWidth,
                              int clientHeight, int* fullWidth, int* fullHeight)
{
...
static GLFWbool acquireMonitor(_GLFWwindow* window)
{
    ...
    SetWindowPos(window->win32.handle, HWND_TOPMOST, xpos, ypos, mode.width,
                 mode.height, SWP_NOACTIVATE | SWP_NOCOPYBITS);
                                   ...
                                   LPARAM lParam)
{
    ...
    case WM_GETMINMAXINFO:
    {
        ...
        getFullWindowSize(getWindowStyle(window), getWindowExStyle(window), 0,
                          0, &xoff, &yoff);
                              ...
                              const _GLFWwndconfig* wndconfig)
{
    ...
    window->win32.handle =
        CreateWindowExW(exStyle, _GLFW_WNDCLASSNAME, wideTitle, style, xpos,
                                ...
                                const GLFWimage* images)
{
    ...
    SendMessage(window->win32.handle, WM_SETICON, ICON_SMALL,
                (LPARAM)smallIcon);
    ...
    if (handle)
    {
        ...
        if (window)
        {
            ...
            if (!lshiftDown && window->keys[GLFW_KEY_LEFT_SHIFT] == 1)
                _glfwInputKey(window, GLFW_KEY_LEFT_SHIFT, 0, GLFW_RELEASE,
                              mods);

            if (!rshiftDown && window->keys[GLFW_KEY_RIGHT_SHIFT] == 1)
                _glfwInputKey(window, GLFW_KEY_RIGHT_SHIFT, 0, GLFW_RELEASE,
                              mods);
        }
    }
...
const char* _glfwPlatformGetKeyName(int key, int scancode)
{
    ...
    if (!WideCharToMultiByte(CP_UTF8, 0, name, -1, _glfw.win32.keyName,
                             sizeof(_glfw.win32.keyName), NULL, NULL))
    {
        return NULL;
    }
