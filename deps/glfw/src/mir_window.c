...
/* FIXME Soon to be changed upstream mir! So we can use an egl config to figure
   out the best pixel format!
*/
...
static void handleKeyEvent(const MirKeyboardEvent* key_event,
                           _GLFWwindow* window)
{
    ...
    const int pressed =
        action == mir_keyboard_action_up ? GLFW_RELEASE : GLFW_PRESS;
                                ...
                                const MirPointerEvent* pointer_event)
{
    ...
    // XOR our old button states our new states to figure out what was added or
    // removed
...
static void handleInput(const MirInputEvent* input_event, _GLFWwindow* window)
{
    ...
    case mir_input_event_type_pointer:
        handlePointerEvent(mir_input_event_get_pointer_event(input_event),
                           window);
        break;
    default:
        break;
...
static void addNewEvent(MirSurface* surface, const MirEvent* event,
                        void* context)
{
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    if (window->monitor)
    {
        ...
        mir_surface_set_state(window->mir.surface,
...
{
    return mir_surface_get_visibility(window->mir.surface) ==
...
void _glfwPlatformWaitEventsTimeout(double timeout)
{
    ...
    if (emptyEventQueue(_glfw.mir.event_queue))
    {
        ...
        pthread_cond_timedwait(&_glfw.mir.event_cond, &_glfw.mir.event_mutex,
                               &time);
    }
                              ...
                              int xhot, int yhot)
{
    ...
    stream =
        mir_connection_create_buffer_stream_sync(_glfw.mir.connection, i_w, i_h,
                                                 pixel_format,
                                                 mir_buffer_usage_software);

    cursor->mir.conf =
        mir_cursor_configuration_from_buffer_stream(stream, xhot, yhot);
...
void _glfwPlatformSetCursor(_GLFWwindow* window, _GLFWcursor* cursor)
{
    ...
    {
        mir_wait_for(mir_surface_configure_cursor(window->mir.surface,
                                                  cursor->mir.conf));
        if (cursor->mir.custom_cursor)
        {
                                                      ...
                                                      uint32_t queuefamily)
{
    ...
    return vkGetPhysicalDeviceMirPresentationSupportKHR(device, queuefamily,
