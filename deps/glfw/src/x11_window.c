                                   ...
                                   Visual* visual, int depth)
{
    ...
    // Create the actual window
    {
        ...
        window->x11.handle =
            XCreateWindow(_glfw.x11.display, _glfw.x11.root, 0, 0,
    ...
    if (_glfw.x11.im)
    {
        window->x11.ic = XCreateIC(_glfw.x11.im, XNInputStyle,
                                   ...
                                   XNFocusWindow, window->x11.handle, NULL);
    }
...
static Atom writeTargetToProperty(const XSelectionRequestEvent* request)
{
    ...
    if (request->target == _glfw.x11.TARGETS)
    {
        ...
        const Atom targets[] = {_glfw.x11.TARGETS, _glfw.x11.MULTIPLE,
                                ...
                                _glfw.x11.COMPOUND_STRING, XA_STRING};

        XChangeProperty(_glfw.x11.display, request->requestor,
    ...
    if (request->target == _glfw.x11.MULTIPLE)
    {
        ...
        XChangeProperty(_glfw.x11.display, request->requestor,
                        request->property, _glfw.x11.ATOM_PAIR, 32,
    ...
    if (request->target == _glfw.x11.SAVE_TARGETS)
    {
        ...
        XChangeProperty(_glfw.x11.display, request->requestor,
                        request->property, _glfw.x11.NULL_, 32, PropModeReplace,
                        NULL, 0);
...
static unsigned int decodeUTF8(const char** s)
{
    ...
    static const unsigned int offsets[] = {0x00000000u, 0x00003080u,
                                           0x000e2080u, 0x03c82080u,
...
static void processEvent(XEvent* event)
{
        ...
        if (window->x11.ic)
        {
            ...
            if (!filtered)
            {
                ...
                if (status == XBufferOverflow)
                {
                    ...
                    count = Xutf8LookupString(window->x11.ic, &event->xkey,
                                              chars, count, NULL, &status);
                }
                ...
                count = XwcLookupString(window->x11.ic, &event->xkey, buffer,
                                        sizeof(buffer) / sizeof(wchar_t), NULL,
                                        &status);

                if (status == XBufferOverflow)
                {
                    chars = calloc(count, sizeof(wchar_t));
                    count = XwcLookupString(window->x11.ic, &event->xkey, chars,
                                            count, NULL, &status);
                }
    ...
    case ConfigureNotify:
    {
            ...
            event->xconfigure.y != window->x11.ypos)
        {
            ...
            {
                _glfwInputWindowPos(window, event->xconfigure.x,
                                    event->xconfigure.y);
    ...
    case ClientMessage:
    {
        ...
        if (event->xclient.message_type == _glfw.x11.WM_PROTOCOLS)
        {
            ...
            {
                // The window manager was asked to close the window, for example
                // by the user pressing a 'close' window decoration button
                                        ...
                                        unsigned char** value)
{
    ...
    XGetWindowProperty(_glfw.x11.display, window, property, 0, LONG_MAX, False,
                       type, &actualType, &actualFormat, &itemCount,
                       &bytesAfter, value);
...
void _glfwPlatformDestroyWindow(_GLFWwindow* window)
{
    ...
    if (window->x11.handle)
    {
        ...
        XDeleteContext(_glfw.x11.display, window->x11.handle,
                       _glfw.x11.context);
...
void _glfwPlatformSetWindowTitle(_GLFWwindow* window, const char* title)
{
    ...
    XmbSetWMProperties(_glfw.x11.display, window->x11.handle, title, title,
                       NULL, 0, NULL, NULL, NULL);
#endif
                                     ...
                                     int* right, int* bottom)
{
        ...
        _glfw.x11.NET_REQUEST_FRAME_EXTENTS)
    {
        ...
        sendEventToWM(window, _glfw.x11.NET_REQUEST_FRAME_EXTENTS, 0, 0, 0, 0,
                      0);
    ...
    if (_glfwGetWindowPropertyX11(window->x11.handle,
                                  _glfw.x11.NET_FRAME_EXTENTS, XA_CARDINAL,
...
void _glfwPlatformIconifyWindow(_GLFWwindow* window)
{
    if (window->x11.overrideRedirect)
    {
        ...
        _glfwInputError(GLFW_PLATFORM_ERROR,
                        "X11: Iconification of full screen windows requires a "
                        "WM that supports EWMH full screen");
        return;
    }
...
const char* _glfwPlatformGetKeyName(int key, int scancode)
{
    ...
    XkbTranslateKeySym(_glfw.x11.display, &keysym, 0, _glfw.x11.keyName,
                       sizeof(_glfw.x11.keyName), &extra);
