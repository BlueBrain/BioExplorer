...
static void usage(void)
{
    ...
    printf(
        ...
        "(" API_NAME_NATIVE " or " API_NAME_EGL ")\n");
...
static void list_vulkan_instance_extensions(void)
{
    ...
    if (vkEnumerateInstanceExtensionProperties(NULL, &ep_count, NULL) !=
        VK_SUCCESS)
        return;

    ep = calloc(ep_count, sizeof(VkExtensionProperties));

    if (vkEnumerateInstanceExtensionProperties(NULL, &ep_count, ep) !=
        VK_SUCCESS)
    {
        free(ep);
        return;
...
static void list_vulkan_instance_layers(void)
{
    ...
    PFN_vkEnumerateInstanceLayerProperties vkEnumerateInstanceLayerProperties =
        (PFN_vkEnumerateInstanceLayerProperties)glfwGetInstanceProcAddress(
                                          ...
                                          VkPhysicalDevice device)
{
    ...
    if (vkEnumerateDeviceExtensionProperties(device, NULL, &ep_count, NULL) !=
        VK_SUCCESS)
        return;

    ep = calloc(ep_count, sizeof(VkExtensionProperties));

    if (vkEnumerateDeviceExtensionProperties(device, NULL, &ep_count, ep) !=
        VK_SUCCESS)
    {
        free(ep);
        return;
                                      ...
                                      VkPhysicalDevice device)
{
    ...
    PFN_vkEnumerateDeviceLayerProperties vkEnumerateDeviceLayerProperties =
        (PFN_vkEnumerateDeviceLayerProperties)glfwGetInstanceProcAddress(
...
int main(int argc, char** argv)
{
    ...
    while ((ch = getopt_long(argc, argv, "a:b:c:dfhlm:n:p:s:v", options,
                             NULL)) != -1)
    {
        switch (ch)
        {
    ...
    if (client == GLFW_OPENGL_ES_API ||
        glfwExtensionSupported("GL_ARB_multisample") || major > 1 || minor >= 3)
    {
