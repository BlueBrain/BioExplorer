                                  ...
                                  int mods)
{
    ...
    printf("%08x to %i at %0.3f: Mouse button %i (%s) (with%s) was %s\n",
           ...
           get_button_name(button), get_mods_name(mods),
                         ...
                         int mods)
{
    ...
    {
        printf(
            "%08x to %i at %0.3f: Key 0x%04x Scancode 0x%04x (%s) (%s) "
            "(with%s) was %s\n",
            counter++, slot->number, glfwGetTime(), key, scancode,
            get_key_name(key), name, get_mods_name(mods),
            get_action_name(action));
    }
    else
    {
                               ...
                               int mods)
{
    Slot* slot = glfwGetWindowUserPointer(window);
    printf(
        "%08x to %i at %0.3f: Character 0x%08x (%s) with modifiers (with%s) "
        "input\n",
...
static void monitor_callback(GLFWmonitor* monitor, int event)
{
    if (event == GLFW_CONNECTED)
    {
        ...
        printf(
            "%08x at %0.3f: Monitor %s (%ix%i at %ix%i, %ix%i mm) was "
            "connected\n",
...
static void joystick_callback(int joy, int event)
{
    if (event == GLFW_CONNECTED)
    {
        ...
        printf(
            "%08x at %0.3f: Joystick %i (%s) was connected with %i axes and %i "
            "buttons\n",
            counter++, glfwGetTime(), joy, glfwGetJoystickName(joy), axisCount,
            buttonCount);
    }
    else
    {
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (monitor)
        {
            printf("Creating full screen window %i (%ix%i on %s)\n",
                   slots[i].number, width, height, glfwGetMonitorName(monitor));
        }
        else
        {
        ...
        glfwSetFramebufferSizeCallback(slots[i].window,
