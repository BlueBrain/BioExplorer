...
static void draw_joystick(int index, int x, int y, int width, int height)
{
    ...
    if (axis_count)
    {
        ...
        for (i = 0; i < axis_count; i++)
        {
            ...
            glRecti(x + i * axis_width, y, x + (i + 1) * axis_width,
                    y + axis_height);
    ...
    if (button_count)
    {
        ...
        for (i = 0; i < button_count; i++)
        {
            ...
            glRecti(x + i * button_width, y + axis_height,
                    x + (i + 1) * button_width,
...
static void draw_joysticks(GLFWwindow* window)
{
    ...
    {
        draw_joystick(i, 0, offset * height / joystick_count, width,
...
static void joystick_callback(int joy, int event)
{
    if (event == GLFW_CONNECTED)
    {
        ...
        printf("Found joystick %i named \'%s\' with %i axes, %i buttons\n",
               joy + 1, glfwGetJoystickName(joy), axis_count, button_count);
