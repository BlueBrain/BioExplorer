...
#define PLUGIN_INFO(__level, __msg)                                    \
    if (__level <= bioexplorer::common::GeneralSettings::getInstance() \
                       ->getLoggingLevel())                            \
...
#define PLUGIN_TIMER(__time, __msg)                                    \
    if (bioexplorer::common::GeneralSettings::getInstance()            \
            ->getLoggingLevel() > 0)                                   \
        std::cout << "T [" << PLUGIN_PREFIX << "] [" << __time << "] " \
                  << __msg << std::endl;
...
#define PLUGIN_DB_DEBUG(__msg)                                    \
    if (bioexplorer::common::GeneralSettings::getInstance()       \
            ->getDBLoggingLevel() > 0)                            \
    {                                                             \
        std::cout << "D [" << PLUGIN_PREFIX << "] [DB] " << __msg \
                  << std::endl;                                   \
    }
#endif
#define PLUGIN_DB_TIMER(__time, __msg)                                      \
    if (bioexplorer::common::GeneralSettings::getInstance()                 \
            ->getDBLoggingLevel() > 0)                                      \
        std::cout << "T [" << PLUGIN_PREFIX << "] [DB] [" << __time << "] " \
                  << __msg << std::endl;
    ...
    {                                                                         \
        ...
        {                                                                     \
            ...
            const uint32_t __pos =                                            \
                std::min(PROGRESS_BAR_SIZE,                                   \
                         int(__p / __mv * PROGRESS_BAR_SIZE));                \
