...
class BioExplorer:
  ...
  def __init__(self, url=None):
    ...
    if __version__ != backend_version:
      raise RuntimeError("Wrong version of the back-end (" + backend_version +
  ...
  def version(self):
    ...
    result = self._client.rockets_client.request(method=self.PLUGIN_API_PREFIX +
                                                 "version")
  ...
  def export_brick_to_db(
    ...
    result = self._client.rockets_client.request(
        method=self.PLUGIN_API_PREFIX + "export-brick-to-db", params=params)
    if not result["status"]:
      raise RuntimeError(result["contents"])
    return result
  ...
  def add_coronavirus(
    ...
    coronavirus = Virus(
        ...
        assembly_params=Vector3(radius, 1.5, morphing_step),
    )
  ...
  def add_virus(
    ...
    if virus.protein_s:
      ...
      if _protein_s.instance_indices[0]:
        ...
        _protein_s_open = AssemblyProtein(
            ...
            assembly_params=Vector3(radius, _protein_s.assembly_params.y,
                                    morphing_step),
            atom_radius_multiplier=atom_radius_multiplier,
    ...
    if virus.protein_m:
      ...
      _protein_m = AssemblyProtein(
          ...
          assembly_params=Vector3(radius, virus.protein_m.assembly_params.y,
                                  morphing_step),
          atom_radius_multiplier=atom_radius_multiplier,
    ...
    if virus.protein_e:
      ...
      _protein_e = AssemblyProtein(
          ...
          assembly_params=Vector3(radius, virus.protein_e.assembly_params.y,
                                  morphing_step),
          atom_radius_multiplier=atom_radius_multiplier,
  ...
  def add_cell(
    ...
    self.add_membrane(
        ...
        shape=cell.shape,
        assembly_params=Vector3(cell.size.x, cell.size.y, 0.0),
  ...
  def add_volume(
    ...
    _protein = AssemblyProtein(
        ...
        assembly_params=volume.size,
        atom_radius_multiplier=atom_radius_multiplier,
        load_bonds=False,
        representation=representation,
        random_seed=random_seed,
        position=volume.protein.position,
        orientation=volume.protein.orientation,
    )
  ...
  def get_protein_amino_acid_information(self, assembly_name, name):
    ...
    params["name"] = name
    result = self._client.rockets_client.request(
        method=self.PLUGIN_API_PREFIX + "get-protein-amino-acid-information",
        params=params,
    )
    if not result["status"]:
      raise RuntimeError(result["contents"])
  ...
  def set_protein_amino_acid(self,
                             assembly_name,
                             name,
                             index,
                             amino_acid_short_name,
                             chain_ids=list()):
    """
        Displays a specified amino acid sequence on the protein
    ...
    params["chainIds"] = chain_ids
    result = self._client.rockets_client.request(
        method=self.PLUGIN_API_PREFIX + "set-protein-amino-acid", params=params)
    if not result["status"]:
      raise RuntimeError(result["contents"])
    self._client.set_renderer(accumulation=True)
    return result
  ...
  def add_membrane(
    ...
    assert isinstance(assembly_params, Vector3)
    ...
    for path in paths:
      ...
      if site_indices:
        _glycans = Sugars(
            ...
            name=assembly_name + "_" + protein_name + "_" + glycan_type + "_" +
            str(path_index),
            source=path,
  ...
  def set_general_settings(self,
    ...
    """
        ...
        :database_connection_string: Connection string to a database (if applicable)
        :database_schema: Database schema (if applicable)
        :bricks_folder: Folder where out-of-core bricks files are stored
        :return: Result of the request submission
        """
    params = dict()
    params["modelVisibilityOnCreation"] = model_visibility_on_creation
    params["databaseConnectionString"] = database_connection_string
...
class AssemblyMeshBasedMembrane:
  ...
  def __init__(
      ...
      recenter=True,
      density=1,
      surface_fixed_offset=0,
      surface_variable_offset=0,
      atom_radius_multiplier=1.0,
      representation=BioExplorer.REPRESENTATION_ATOMS,
      random_seed=0,
      position=Vector3(),
