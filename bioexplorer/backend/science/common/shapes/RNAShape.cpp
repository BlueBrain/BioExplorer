...
{
    _bounds.merge(Vector3d(-curveParams.x / 2.0, -curveParams.y / 2.0, -curveParams.z / 2.0));
    _bounds.merge(Vector3d(curveParams.x / 2.0, curveParams.y / 2.0, curveParams.z / 2.0));
...
Transformation RNAShape::getTransformation(const uint64_t occurrence, const uint64_t nbOccurrences,
...
void RNAShape::_getSegment(const double u, const double v, Vector3d& src, Vector3d& dst) const
{
    switch (_shapeType)
    {
    case RNAShapeType::moebius:
...
{
    return {_shapeParams.x * (cos(t) + _curveParams.x * cos(_curveParams.y * t) * cos(t)),
            _shapeParams.x * (sin(t) + _curveParams.x * cos(_curveParams.y * t) * sin(t)),
            _shapeParams.x * _curveParams.x * sin(_curveParams.y * t)};
}

Vector3d RNAShape::_star(const double t) const
{
    return {_shapeParams.x * (2.0 * sin(3.0 * t) * cos(t)), _shapeParams.x * (2.0 * sin(3.0 * t) * sin(t)),
...
Vector3d RNAShape::_heart(const double u) const
{
    return {_shapeParams.x * 4.0 * pow(sin(u), 3.0),
            _shapeParams.x * 0.25 * (13.0 * cos(u) - 5.0 * cos(2.0 * u) - 2.0 * cos(3.0 * u) - cos(4.0 * u)), 0.0};
}

Vector3d RNAShape::_thing(const double t) const
{
    return {_shapeParams.x * (sin(t) + _curveParams.x * sin(_curveParams.y * t)),
            _shapeParams.x * (cos(t) - _curveParams.x * cos(_curveParams.y * t)),
            _shapeParams.x * (-sin(_curveParams.z * t))};
}
...
Vector3d RNAShape::_moebius(const double u, const double v) const
{
    return {4.0 * _shapeParams.x * (cos(u) + v * cos(u / 2.0) * cos(u)),
            4.0 * _shapeParams.x * (sin(u) + v * cos(u / 2.0) * sin(u)), 8.0 * _shapeParams.x * (v * sin(u / 2.0))};
}
