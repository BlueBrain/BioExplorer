...
void ScreenSpaceProcessor::init(uint32_t width, uint32_t height)
{
    ...
    if (!linkProgram(quadShader_, vertQuad_glsl, fragQuad_glsl))
        BRAYNS_ERROR("Error: cannot compile shader");
...
void ScreenSpaceProcessor::resize(uint32_t width, uint32_t height)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, &emptyData[0]);

    glBindTexture(GL_TEXTURE_2D, depthTexture_);

    std::vector<float> emptyDataFloat(width * height, 0u);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width, height, 0, GL_LUMINANCE, GL_FLOAT, &emptyDataFloat[0]);
...
void ScreenSpaceProcessor::draw(const ScreenSpaceProcessorData& buffersData)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, buffersData.width, buffersData.height, buffersData.colorFormat,
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, buffersData.width, buffersData.height, buffersData.depthFormat,
...
void ScreenSpaceProcessor::createDepthTexture_(uint32_t width, uint32_t height)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width, height, 0, GL_LUMINANCE, GL_FLOAT,
                 &emptyData[0]); // Maybe GL_DEPTH_COMPONENT.
