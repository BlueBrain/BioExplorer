...
bool linkProgram(const unsigned program, const char* vertexShaderSource, const char* fragmentShaderSource)
{
    ...
    if (!status)
    {
        ...
        BRAYNS_ERROR("Failed to link shader program " << program << ": " << errorLog);
        return false;
    }
    return true;
}
