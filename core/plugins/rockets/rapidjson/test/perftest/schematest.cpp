...
public:
    ...
    virtual void SetUp()
    {
        ...
        for (size_t i = 0; i < ARRAY_SIZE(filenames); i++)
        {
                 ...
                 schemaItr != d.End(); ++schemaItr)
            {
                ...
                {
                    if (IsExcludeTest(schemaDescription + ", " +
    ...
    {
        PerfTest::TearDown();
        for (TestSuiteList::const_iterator itr = testSuites.begin();
             itr != testSuites.end(); ++itr)
            delete *itr;
        testSuites.clear();
    }

private:
    // Using the same exclusion in
    ...
    static bool IsExcludeTestSuite(const std::string& description)
    {
        const char* excludeTestSuites[] = {
            ...
            "remote ref", "remote ref, containing refs itself",
            "fragment within remote ref", "ref within remote ref",
            "change resolution scope",
            // these below were added to get jsck in the benchmarks)
            "uniqueItems validation", "valid definition", "invalid definition"};
    ...
    static bool IsExcludeTest(const std::string& description)
    {
        const char* excludeTests[] = {
            ...
            "maxLength validation, two supplementary Unicode code points is "
            "long enough",
            "minLength validation, one supplementary Unicode code point is not "
            "long enough",
...
protected:
    ...
    struct TestSuite
    {
        ...
        {
            delete schema;
            for (DocumentList::iterator itr = tests.begin(); itr != tests.end();
                 ++itr)
                delete *itr;
        }
...
TEST_F(Schema, TestSuite)
{
    ...
    for (int i = 0; i < trialCount; i++)
    {
             ...
             itr != testSuites.end(); ++itr)
        {
            ...
            GenericSchemaValidator<SchemaDocument, BaseReaderHandler<UTF8<>>,
                                   MemoryPoolAllocator<>>
                validator(*ts.schema, &validatorAllocator);
            for (DocumentList::const_iterator testItr = ts.tests.begin();
                 testItr != ts.tests.end(); ++testItr)
            {
                validator.Reset();
