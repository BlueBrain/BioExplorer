...
int main(int, char*[])
{
    ...
    assert(document["i"].IsInt());    // In this case,
                                   // IsUint()/IsInt64()/IsUInt64() also return
                                   // true.
    printf("i = %d\n",
    ...
    // Adding values to array.
    {
        ...
        for (int i = 5; i <= 10; i++)
            a.PushBack(i, allocator); // May look a bit strange, allocator is
                                      // needed for potentially realloc. We
    ...
    assert(author.IsNull()); // Move semantic for assignment. After this
                             // variable is assigned as a member, the variable
                             // becomes null.
    ...
    document.Accept(writer); // Accept() traverses the DOM and generates Handler
                             // events.
    puts(sb.GetString());
