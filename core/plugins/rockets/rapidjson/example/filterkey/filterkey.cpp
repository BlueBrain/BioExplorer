...
public:
    ...
    bool Null()
    {
        return filterValueDepth_ > 0 ? EndValue()
    ...
    {
        return filterValueDepth_ > 0
                   ? EndValue()
                   : outputHandler_.RawNumber(str, len, copy) && EndValue();
    }
    bool String(const Ch* str, SizeType len, bool copy)
    {
        return filterValueDepth_ > 0
                   ? EndValue()
                   : outputHandler_.String(str, len, copy) && EndValue();
    }
...
int main(int argc, char* argv[])
{
    ...
    // events, and forward to writer. i.e. the events flow is: reader -> filter
    // -> writer
