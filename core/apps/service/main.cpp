...
public:
        ...
        , _stopRenderThread{_renderLoop->resource<uvw::AsyncHandle>()}
    {
        ...
        CORE_INFO("Events from rockets, trigger rendering");
        _brayns->getEngine().triggerRender = [&eventRendering = _eventRendering] { eventRendering->start(); };

        CORE_INFO("Launch first frame; after that, only events will trigger that");
        _eventRendering->start();

        CORE_INFO("Stop the application on Ctrl+C");
...
private:
    ...
    const float _idleRenderingDelay{1.0f};
